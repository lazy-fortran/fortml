...
| Dense MLP and MSE training objective | Parameter/input JVPs, VJPs, exact MSE+L2 HVPs, L2 hyperparameter derivative, Adam/AdamW/Adagrad/RMSprop/AMSGrad/RAdam/SGD momentum/Nesterov training, typed constant/warmup/cosine/exponential/one-cycle schedules with analytic rate products, exact fixed-trajectory learning-rate/L2, one-cycle peak/final schedule products, scheduled RAdam base-rate/beta/epsilon/schedule hypergradients, AdamW (including beta logits), RMSprop hypergradients, AMSGrad/RAdam recurrence/checkpoints, exact in-memory optimizer checkpoints, and exact outer SGD momentum/Nesterov HVPs for one-layer affine MLPs | Mini-batch/schedule optimizer-group/validation-policy/device-state hypergradients and neural module families are partial |
