...
| --- | --- | --- |
| Regression and features | `fortml_linear_regression`, `fortml_preprocessing`, `fortml_basis`, `fortml_pipeline`, `fortml_column_pipeline`, `fortml_validation` | Dense SVD fit plus fitted standard/min-max scalers with exact input JVPs. Horizontal, sequential, and explicit column-selecting basis pipelines plus deterministic K-fold/stratified split iterators are public. |
| Classification | `fortml_logistic_regression`, `fortml_softmax_regression`, `fortml_mlp_classifier`, `fortml_gp_classification`, `fortml_gp_multiclass_classification`, `fortml_classification_metrics`, `fortml_losses` | Linear and neural classifiers support nonnegative sample weights and positive sorted-class weights. Binary Laplace GP and one-vs-rest multiclass GP classifiers use stable probabilities and arbitrary integer labels. Shared accuracy, top-k, balanced accuracy, confusion, precision/recall/F1, Brier, binary Matthews, weighted accuracy, and log-loss metrics are implemented. Variational categorical GP likelihoods remain planned. |
| Neural models | `fortml_mlp`, `fortml_mlp_training`, `fortml_mlp_classifier`, `fortml_bnn`, `fortml_vae`, `fortml_rnn` | MLPs use deterministic Xavier/He initialization, Adam training, explicit seeded batch cursors, sample-weighted microbatch accumulation, held-out validation with interval monitoring and best-state restoration, user schedules, norm clipping, exact MSE+L2 parameter/hyperparameter HVPs, and a multiclass logits classifier. The recurrent model is one vanilla `tanh` RNN with a zero initial state |
| Trees and boosting | `fortml_tree`, `fortml_xgboost`, `fortml_xgboost_multiclass` | Deterministic finite-only regression stumps, squared-loss residual boosting, exact depth-one second-order squared/logistic boosting with L1/L2/gamma/min-child-Hessian regularization, and one-vs-rest multiclass probabilities with quotient-rule JVPs. Deeper/histogram/CART/missing-value/ranking/constraint policies remain planned. |
...
| Exact GPs | `fortml_kernels`, `fortml_gaussian_process`, `fortml_gp_training`, `fortml_derivative_gaussian_process`, `fortml_derivative_gp_training`, `fortml_multi_output_gp` | Exact-GP hyperparameters and mixed value/first-derivative GP states expose bounded FortOpt adapters. Derivative-GP prediction parameter JVP/VJP products cover fixed value/first-derivative queries; query-input and joint-covariance products remain roadmap work. |
