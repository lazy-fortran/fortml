...
| Dense MLP and MSE training objective | Parameter/input JVPs, VJPs, exact MSE+L2 HVPs, L2 hyperparameter derivative, Adam/AdamW/Adagrad/RMSprop/AMSGrad/RAdam/SGD momentum/Nesterov training, typed constant/warmup/cosine/exponential/one-cycle schedules with analytic rate products, exact fixed-trajectory learning-rate/L2, one-cycle peak/final schedule products, AdamW (including beta logits), RMSprop hypergradients, AMSGrad/RAdam recurrence/checkpoints, exact in-memory optimizer checkpoints, and exact outer SGD momentum/Nesterov HVPs for one-layer affine MLPs | Mini-batch/schedule optimizer-group/validation-policy/device-state hypergradients and neural module families are partial |
...
The classification surface also includes `fortml_calibrated_softmax_classifier`
...
packed/input JVP/VJP products plus typed CUDA refusal. The independent release
lane is `results/GP_VARIATIONAL_CATEGORICAL.md` in `../fortml-bench`.
Additional variational categorical likelihood families and resident GPU
