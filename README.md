...
regression (including weighted Bayesian-ridge posterior inference), binary classification, neural models, exact and approximate
...
`fortml_ovo_rbf_svm_classifier` adds deterministic one-vs-one dense RBF-SVM
classification over arbitrary sorted integer labels. Each pair retains its
own finite basis and packed parameter offset; normalized pairwise-vote
probabilities, decision/probability input and parameter JVP/VJP products,
transactional fit, CPU dispatch, and explicit CUDA refusals are covered by an
independent Fortran gate and NumPy/SciPy benchmark. See
[`docs/RBF_SVM_OVO.md`](docs/RBF_SVM_OVO.md).
...
| --- | --- | --- |
| Linear, ridge, elastic-net, and Bayesian-ridge estimators, scalers, basis maps, and basis pipelines | Value, input/parameter JVPs and VJPs; fixed-hyperparameter Bayesian-ridge posterior mean/precision/covariance and log-evidence; analytic HVPs for polynomial/Chebyshev/Fourier/radial/spline maps and horizontal/column/sequential/fan-out/residual-sum/conditional interval-routed pipelines; versioned transactional state dictionaries for fitted horizontal unions | Evidence maximisation/ARD, derivative-through-fit, cyclic DAG transforms, callback HVPs, graph/estimator-wide and resident-device serialization remain open |
...
| Derivative-observation GP | Mixed value/first-derivative observations, dense joint latent covariance, analytic parameter JVP/VJP products, exact query-input JVP/VJP products, and CPU through-fit training-target JVP/VJP products for RBF, Matérn 3/2 and 5/2, periodic, local-periodic, rational-quadratic, cosine, linear, constant, and supported sum/product leaves; periodic, local-periodic, rational-quadratic, and Matérn 3/2/5/2 mixed-observation likelihood HVPs now cover all logarithmic kernel/noise coordinates; validated user formulas carry analytic value/gradient/Hessian products; bounded scalar 1-D `second_derivative_gp_t` supports RBF value/gradient/Hessian/third-derivative rows with order-six covariance, order-seven query products, and analytic likelihood gradient/HVPs, while Matérn-5/2 remains at orders `0:2` | Matérn 1/2 coincident derivative blocks, cosine mixed HVPs, user-formula third-input products, higher-order observations beyond RBF order three, through-fit CUDA products, and resident CUDA covariance/solve kernels remain typed refusals |
...
| Regression and robust losses | `fortml_quantile_regression`, `fortml_huber_regression`, `fortml_weighted_ols`, `fortml_ridge_regression`, `fortml_elastic_net_regression`, `fortml_linear_svr`, `fortml_random_forest_regressor` | Weighted multi-output linear quantile regression sorts unique levels deterministically, routes a registry-backed pinball objective through FortOpt L-BFGS-B with an explicit C1 continuation, and exposes packed fixed-state input/parameter JVP/VJP products plus exact-kink HVP refusals. `random_forest_regressor_t` adds a transactional seeded weighted bootstrap CART ensemble for scalar/multi-output targets, staged prefix predictions, bootstrap inclusion metadata, normalized split-frequency feature importance, and exact zero input JVP/VJP products away from split boundaries. See [`docs/QUANTILE_REGRESSION.md`](docs/QUANTILE_REGRESSION.md) and [`docs/RANDOM_FOREST_REGRESSION.md`](docs/RANDOM_FOREST_REGRESSION.md). CUDA requests remain typed refusals until resident affine/pinball/regression-forest kernels are linked. |
...
`fortml_mlp_structure_gp` adds the ordinary fixed-depth structure-aware path.
It captures the packed hidden parameter prefix and its RMS initialization
scale, then refuses any topology, layout, hidden-state, or scale change before
applying the finite-feature posterior. Only the final affine layer is
mutated. The independent contract and CUDA boundary are documented in
[`docs/MLP_STRUCTURE_GP.md`](docs/MLP_STRUCTURE_GP.md).
