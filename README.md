...
| Variational inference | `fortml_variational`, `fortml_sparse_gp`, `fortml_gp_variational_classification` | `sparse_gp_t` has scalar Gaussian targets, packed mean/log-Cholesky variational parameters, exact ELBO gradients/JVPs/VJPs, and typed CPU/CUDA dispatch; `gp_variational_classification_t` adds bounded Bernoulli logistic/probit ELBOs, inducing `q(u)`, packed gradients, deterministic JVPs, minibatch scaling, and an explicit CUDA refusal |
