...
| Neural initialization | `fortml_mlp_last_layer_gp`, `fortml_mlp_structure_gp` | The finite-feature last-layer initializer solves a deterministic regularized kernel-ridge output layer. The structure-aware wrapper records the ordinary fixed-depth MLP hidden parameter prefix, packed layout, and RMS scale, validates them before every apply/query, and mutates only the final affine layer. Both paths expose fixed-feature products, independent oracles, and typed CUDA refusals. They are finite-feature warm starts, not NNGP or exact infinite-width claims. |
