...
| --- | --- | --- |
| Regression and features | `fortml_linear_regression`, `fortml_regression_metrics`, `fortml_preprocessing`, `fortml_basis`, `fortml_pipeline`, `fortml_column_pipeline`, `fortml_validation` | Dense SVD fit plus core weighted regression metrics, fitted standard/min-max scalers with exact input JVPs, and horizontal, sequential, and explicit column-selecting basis pipelines. Deterministic K-fold/stratified split iterators are public. |
...
| Trees and boosting | `fortml_tree`, `fortml_xgboost`, `fortml_xgboost_multiclass` | Deterministic finite-only regression stumps, a weighted exhaustive-split depth-limited CART regressor with zero-away-from-boundary JVPs, squared-loss residual boosting, exact depth-one second-order squared/logistic boosting with L1/L2/gamma/min-child-Hessian regularization, and one-vs-rest multiclass probabilities with quotient-rule JVPs. Histograms, CART classification, missing-value/ranking/constraint policies remain planned. |
