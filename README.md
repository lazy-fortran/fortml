...
| Neural initialization | `fortml_mlp_last_layer_gp`, `fortml_mlp_structure_gp`, `fortml_pinn_structure_gp` | The finite-feature last-layer initializer solves a deterministic regularized kernel-ridge output layer. The ordinary and PINN structure-aware wrappers record fixed-depth hidden prefixes, packed layouts, RMS scales, named physics terms where applicable, validate them before every apply/query, and mutate only the final affine layer. CPU products have independent oracles; CUDA paths are typed refusals. These are finite-feature warm starts, not NNGP or exact infinite-width claims. |
