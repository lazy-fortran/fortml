...
| Regression and robust losses | `fortml_quantile_regression`, `fortml_huber_regression`, `fortml_weighted_ols`, `fortml_ridge_regression`, `fortml_elastic_net_regression`, `fortml_linear_svr` | Weighted multi-output linear quantile regression sorts unique levels deterministically, routes a registry-backed pinball objective through FortOpt L-BFGS-B with an explicit C1 continuation, and exposes packed fixed-state input/parameter JVP/VJP products plus exact-kink HVP refusals. See [`docs/QUANTILE_REGRESSION.md`](docs/QUANTILE_REGRESSION.md). CUDA requests remain typed refusals until resident affine/pinball kernels are linked. |
