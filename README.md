...
| Trees and boosting | `fortml_tree`, `fortml_cart_classifier`, `fortml_xgboost`, `fortml_xgboost_multiclass` | Deterministic finite-only regression stumps, a weighted exhaustive-split depth-limited CART regressor with zero-away-from-boundary JVPs, weighted Gini/entropy CART classification with leaf probabilities, squared-loss residual boosting, exact depth-one second-order squared/logistic boosting with L1/L2/gamma/min-child-Hessian regularization, and one-vs-rest multiclass probabilities with quotient-rule JVPs. Histograms, missing-value/ranking/constraint policies remain planned. |
