program test_mlp_radam_schedule_hypergradient
    ...
    use fortml_mlp_radam_schedule_hypergradient, only: &
        mlp_radam_schedule_hypergradient_objective_t, mlp_radam_schedule_hypergradient_options_t, &
        mlp_radam_schedule_hypergradient_metadata_t, mlp_radam_schedule_hypergradient_result_t, &
        mlp_optimize_radam_schedule_hyperparameters, MLP_RADAM_SCHEDULE_HYPERPARAMETER_COUNT, &
        MLP_RADAM_SCHEDULE_LOG_BASE_RATE, MLP_RADAM_SCHEDULE_LOG_L2, MLP_RADAM_SCHEDULE_LOGIT_BETA1, &
        MLP_RADAM_SCHEDULE_LOGIT_BETA2, MLP_RADAM_SCHEDULE_LOG_EPSILON

    implicit none

    type(mlp_t), target :: model
    type(mlp_radam_schedule_hypergradient_objective_t) :: objective
    type(mlp_radam_schedule_hypergradient_options_t) :: options, bad_options
    type(mlp_radam_schedule_hypergradient_result_t) :: result
    type(mlp_radam_schedule_hypergradient_metadata_t) :: metadata
    ...
    real(dp) :: direction(MLP_RADAM_SCHEDULE_HYPERPARAMETER_COUNT)
    real(dp) :: gradient(MLP_RADAM_SCHEDULE_HYPERPARAMETER_COUNT)
    real(dp) :: vjp_gradient(MLP_RADAM_SCHEDULE_HYPERPARAMETER_COUNT)
    real(dp) :: hvp(MLP_RADAM_SCHEDULE_HYPERPARAMETER_COUNT)
    ...
    call check(metadata%parameter_count == MLP_RADAM_SCHEDULE_HYPERPARAMETER_COUNT .and. &
    ...
    h = 2.0e-6_dp
    do i = 1, MLP_RADAM_SCHEDULE_HYPERPARAMETER_COUNT
    ...
    direction = [0.21_dp, -0.17_dp, 0.23_dp, -0.19_dp, 0.13_dp, 0.11_dp, -0.07_dp]
    ...
    call check(status%code == FORTNUM_NOT_IMPLEMENTED, &
        "RAdam outer HVP typed refusal", failures)

    ! Exercise the second schedule derivative branch independently: the
    ! exponential family uses the decay-factor coordinate rather than the
    ! cosine minimum-rate coordinate.
    options%schedule = make_mlp_schedule_exponential_decay(1, 0.8_dp)
    options%steps = 4
    call objective%initialize(model, train_x, train_target, validation_x, &
        validation_target, options, status)
    call check(status_ok(status), "exponential scheduled RAdam initialization", failures)
    parameters = objective%parameters()
    call objective%value_gradient(parameters, value, gradient, status)
    parameters(7) = parameters(7) + h
    call objective%value_gradient(parameters, value_plus, vjp_gradient, status)
    parameters(7) = parameters(7) - 2.0_dp*h
    call objective%value_gradient(parameters, value_minus, vjp_gradient, status)
    call check(abs(gradient(7) - (value_plus-value_minus)/(2.0_dp*h)) < 4.0e-6_dp, &
        "exponential RAdam decay-factor derivative", failures)
    ...
    call mlp_optimize_radam_schedule_hyperparameters(model, train_x, train_target, validation_x, &
    ...
    write (*, '(a)') "PASS MLP scheduled RAdam hypergradient independent behavioral oracles"
...
end program test_mlp_radam_schedule_hypergradient
