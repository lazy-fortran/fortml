...
if ! command -v nvcc >/dev/null 2>&1; then
    echo "test_cuda_adagrad_state: CUDA compiler unavailable; skipped"
    exit 0
fi
if ! nvidia-smi >/dev/null 2>&1; then
    echo "test_cuda_adagrad_state: CUDA device unavailable; skipped"
    exit 0
fi
...
"$build_dir/test_cuda_adagrad_state"
