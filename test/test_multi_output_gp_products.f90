program test_multi_output_gp_products
    ...
    integer :: failures, i, nparameters
    ...
    theta_plus = theta
    theta_minus = theta
    theta_plus(1) = theta_plus(1) + eps
    theta_minus(1) = theta_minus(1) - eps
    ...
    call model%predict_parameter_jvp(query, unit_direction(theta), mean, mean_dot, status)
...
contains
    ...
    function unit_direction(theta) result(direction)
