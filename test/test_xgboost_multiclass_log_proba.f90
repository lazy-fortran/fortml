program test_xgboost_multiclass_log_proba
    !! Independent behavioral oracle for multiclass XGBoost log probabilities
    !! and fixed-structure leaf-coordinate products.
    ...
    allocate(direction(model%parameter_count()), parameter_bar(model%parameter_count()), &
        ...
        parameter_probability_dot(3, 3), parameter_log_dot(3, 3))
    ...
    if (failures > 0) then
        write (error_unit, '(a,i0)') "FAIL XGBoost multiclass log-probability cases: ", failures
        error stop 1
    end if
    write (*, '(a)') "PASS XGBoost multiclass log-probability independent oracle"

contains

    subroutine independent_log_oracle(model, query, observed, failures)
        type(xgboost_multiclass_t), intent(in) :: model
        ...
        do i = 1, size(query, 1)
            ...
            do j = 1, size(margins, 2)
                positive(i, j) = 1.0_dp/(1.0_dp + exp(-margins(i, j)))
    ...
    subroutine check(condition, description, failures)
        ...
        if (.not. condition) then
            failures = failures + 1
            write (error_unit, '(a)') "  FAIL [xgb-multiclass-log-proba] "//description
        end if
    end subroutine check

end program test_xgboost_multiclass_log_proba
