program test_gp_multilabel_independent_optimizer
    !! Independent finite-difference, adjoint, and transactional oracle for
    !! per-label fixed-state GP hyperparameter optimization.
    ...
    use fortml_gp_multilabel_classification, only: &
        ...
        gp_multilabel_lbfgsb_result_t, gp_multilabel_optimize_independent_lbfgsb
    implicit none
    ...
    call model%fixed_state_independent_value_gradient(parameters, value, gradient, status)
    call check(status_ok(status) .and. size(parameters) == 4, &
        "independent fixed-state objective shape", failures)

    h = 1.0e-5_dp
    call model%fixed_state_independent_value_gradient(parameters + h*direction, value_plus, &
        gradient_plus, status)
    call model%fixed_state_independent_value_gradient(parameters - h*direction, value_minus, &
    ...
    call check(status_ok(status) .and. abs(dot_product(gradient, direction) - &
        ...
        "independent objective directional finite difference", failures)
    ...
    call check(status_ok(status) .and. abs(lhs - rhs) < 2.0e-7_dp, &
        "independent FortOpt objective JVP/VJP duality", failures)
    call check(maxval(abs(objective_gradient - gradient)) < 2.0e-7_dp, &
        "independent objective gradient contract", failures)

    call model%fixed_state_independent_value_gradient(parameters, value, gradient, status)
    call check(status_ok(status), "restore independent parameters", failures)
    call model%fixed_state_independent_value_gradient([0.0_dp], value, gradient_plus, status)
    call check(status%code == FORTNUM_DOMAIN_ERROR .and. &
        maxval(abs(model%parameters() - saved)) < 1.0e-14_dp, &
        "invalid independent update is transactional", failures)
    ...
    call check(status_ok(status) .and. optimizer_result%converged .and. &
        optimizer_result%iterations >= 0, "FortOpt independent GP optimization", failures)

    if (failures > 0) then
        write (error_unit, '(a,i0)') &
            "FAIL multilabel GP independent optimizer cases: ", failures
        error stop 1
    end if
    write (*, '(a)') "PASS multilabel GP independent optimizer independent oracle"
...
contains
    ...
    subroutine check(condition, description, failures)
        ...
        if (.not. condition) then
            failures = failures + 1
            write (error_unit, '(a)') "  FAIL [gp-multilabel-independent] "//description
        end if
    end subroutine check

end program test_gp_multilabel_independent_optimizer
