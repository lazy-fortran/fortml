program test_bernoulli_naive_bayes
    !! Independent analytic checks for weighted Bernoulli Naive Bayes.
    ...
    type(bernoulli_naive_bayes_t) :: model, weighted_model, unfitted
    ...
    real(dp) :: theta_dot(12), theta(12), prior(3), q(3, 3)
    real(dp) :: probabilities(3, 3), probabilities_one(1, 3)
    real(dp) :: nan_query(3, 3), nan_value
    real(dp) :: expected_q(3, 3), expected_prior(3), expected_log(3)
    real(dp), allocatable :: parameters(:)
    integer :: labels(6), classes(3), prediction(3), failures, i, j, c
    real(dp) :: lhs, rhs, step, analytic, normalizer
    ...
    labels = [7, -2, 7, -2, 4, 4]
    query(1, :) = [0.1_dp, 0.2_dp, 0.3_dp]
    query(2, :) = [0.8_dp, 0.7_dp, 0.6_dp]
    query(3, :) = [0.5_dp, 0.25_dp, 0.75_dp]
    ...
    call check(status_ok(status) .and. all(classes == [-2, 4, 7]) .and. &
        model%class_count() == 3 .and. model%feature_count() == 3, &
        "sorted arbitrary class metadata", failures)
    q = model%feature_probabilities()
    expected_q = reshape([ &
        0.25_dp, 0.5_dp, 0.5_dp, &
        0.5_dp, 0.5_dp, 0.5_dp, &
        0.75_dp, 0.5_dp, 0.5_dp], shape(expected_q))
    ...
    call check(maxval(abs(prior - 1.0_dp/3.0_dp)) < 2.0e-14_dp .and. &
        abs(sum(prior) - 1.0_dp) < 2.0e-14_dp, &
        "empirical class priors", failures)
    ...
    call check(status_ok(status) .and. all(prediction == [-2, 7, 4]) .and. &
    ...
    do i = 1, 3
        expected_log = 0.0_dp
        do c = 1, 3
            expected_log(c) = log(1.0_dp/3.0_dp)
            do j = 1, 3
                expected_log(c) = expected_log(c) + query(i, j)*log(expected_q(j, c)) &
                    + (1.0_dp - query(i, j))*log(1.0_dp - expected_q(j, c))
            end do
        end do
    ...
    expected_q = reshape([ &
        1.0_dp/6.0_dp, 0.5_dp, 0.5_dp, &
        0.5_dp, 0.5_dp, 0.5_dp, &
        0.875_dp, 0.5_dp, 0.5_dp], shape(expected_q))
    ...
    if (failures > 0) then
        write (error_unit, '(a,i0)') "FAIL BernoulliNB cases: ", failures
        error stop 1
    end if
    write (*, '(a)') "PASS Bernoulli Naive Bayes independent analytic oracles"
...
contains
    ...
    subroutine check(condition, name, failures)
        ...
        if (.not. condition) then
            failures = failures + 1
            write (error_unit, '(a)') "  FAIL [bernoulli-nb] "//name
        end if
    end subroutine check

end program test_bernoulli_naive_bayes
