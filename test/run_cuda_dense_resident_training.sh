...
if ! command -v nvcc >/dev/null 2>&1; then
    echo "test_cuda_dense_resident_training: CUDA compiler unavailable; skipped"
    exit 0
fi
if ! nvidia-smi >/dev/null 2>&1; then
    echo "test_cuda_dense_resident_training: CUDA device unavailable; skipped"
    exit 0
fi
build_dir=$(mktemp -d /mnt/storage/fortml-cuda-dense-training.XXXXXX)
