program test_xgboost_tweedie
    !! Independent value/gradient/Hessian and fit oracles for reg:tweedie.
    use, intrinsic :: iso_fortran_env, only: real64, error_unit
    use fortml_device, only: fortml_device_t, FORTML_DEVICE_CPU, FORTML_DEVICE_CUDA
    use fortml_xgboost, only: xgboost_t, xgboost_options_t, &
        xgb_tweedie_loss, xgb_tweedie_derivatives
    ...
    if (failures /= 0) then
        write (error_unit, '(i0,a)') failures, " XGBoost Tweedie test(s) failed"
        error stop 1
    end if
    write (*, '(a)') "PASS XGBoost Tweedie independent behavioral oracles"
...
contains
    ...
    subroutine test_derivative_oracle(failures)
        ...
        real(real64) :: value_plus, value_minus, value, power, h
        real(real64) :: expected_gradient(3), expected_hessian(3), first_term, second_term
        integer :: i

        margin = [-0.4_real64, 0.0_real64, 0.7_real64]
        target = [0.0_real64, 1.5_real64, 4.0_real64]
        direction = [0.2_real64, -0.3_real64, 0.5_real64]
        power = 1.4_real64
        call xgb_tweedie_derivatives(margin, target, power, gradient, hessian, status)
        call check(status_ok(status), "Tweedie derivative status", failures)
        expected_gradient = 0.0_real64
        expected_hessian = 0.0_real64
        do i = 1, 3
            first_term = exp((1.0_real64 - power)*margin(i))
            second_term = exp((2.0_real64 - power)*margin(i))
            expected_gradient(i) = -target(i)*first_term + second_term
            expected_hessian(i) = target(i)*(power - 1.0_real64)*first_term + &
                (2.0_real64 - power)*second_term
        end do
        call check(maxval(abs(gradient - expected_gradient)) < 2.0e-14_real64, &
            "Tweedie gradient formula", failures)
        call check(maxval(abs(hessian - expected_hessian)) < 2.0e-14_real64, &
            "Tweedie Hessian formula", failures)
        call xgb_tweedie_loss(margin, target, power, value, status)
        h = 1.0e-6_real64
    ...
    subroutine test_fit_and_prediction_semantics(failures)
        ...
        options%tweedie_variance_power = 1.5_real64
        ...
        call check(status_ok(status), "Tweedie fit/predict status", failures)
        call check(trim(model%objective_name()) == "tweedie", &
            "Tweedie objective name", failures)
        call check(abs(model%objective_parameter_value() - 1.5_real64) < &
            2.0e-14_real64, "Tweedie power metadata", failures)
        call check(abs(model%base_margin() - expected_base) < 2.0e-14_real64, &
            "Tweedie log-mean base margin", failures)
        call check(all(prediction > 0.0_real64), &
            "Tweedie inverse-link predictions are positive", failures)
        call check(maxval(abs(prediction - exp(margin))) < 2.0e-13_real64, &
            "Tweedie inverse-link prediction semantics", failures)
        call check(maxval(abs(staged(:, 1) - prediction)) < 2.0e-13_real64, &
            "Tweedie staged prediction parity", failures)

        alias_options = options
        alias_options%objective = "reg:tweedie"
        call alias_model%fit(x, target, status, alias_options)
        call alias_model%predict(x, prediction, status)
        call check(status_ok(status) .and. trim(alias_model%objective_name()) == "tweedie", &
            "Tweedie objective alias", failures)
    ...
    subroutine test_domain_and_device_contract(failures)
        ...
        options%max_depth = 1
        options%min_child_weight = 0.0_real64
        options%tweedie_variance_power = 1.0_real64
        call model%fit_tweedie(x, target, status, options)
        call check(status%code == FORTNUM_DOMAIN_ERROR, &
            "unit Tweedie power refusal", failures)
        options%tweedie_variance_power = 2.0_real64
        call model%fit_tweedie(x, target, status, options)
        call check(status%code == FORTNUM_DOMAIN_ERROR, &
            "quadratic Tweedie power refusal", failures)
        options%tweedie_variance_power = 1.5_real64
        call model%fit_tweedie(x, bad_target, status, options)
        call check(status%code == FORTNUM_DOMAIN_ERROR, &
            "negative Tweedie target refusal", failures)
        call model%fit_tweedie(x, target, status, options)
        ...
        call check(model%device_supported(FORTML_DEVICE_CPU), &
            "Tweedie CPU capability", failures)
        call check(.not. model%device_supported(FORTML_DEVICE_CUDA), &
            "Tweedie CUDA capability refusal", failures)
        call model%predict_device(cuda, x, prediction, status)
        call check(status%code == FORTNUM_NOT_IMPLEMENTED, &
            "Tweedie CUDA prediction refusal", failures)
        call model%predict_device(cpu, x, prediction, status)
        call check(status_ok(status), "Tweedie CPU device dispatch", failures)
...
end program test_xgboost_tweedie
