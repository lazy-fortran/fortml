program test_fortsym_matern52
    !! Independent numerical oracle for the FortSym-generated Matérn-5/2 leaf.
    !! Neither the value/JVP nor the reverse tangent oracle calls generated code.
    use, intrinsic :: iso_fortran_env, only: real64, error_unit
    use fortml_generated_matern52_products, only: fortml_matern52_hvp
    implicit none

    integer, parameter :: dp = real64
    real(dp), parameter :: kappa = 2.2360679774997896964_dp
    ...
    expected = matern52_value(distance, lv, ll)
    expected_d = (matern52_value(distance + h*distance_d, lv + h*lv_d, &
        ll + h*ll_d) - matern52_value(distance - h*distance_d, lv - h*lv_d, &
        ll - h*ll_d))/(2.0_dp*h)
    ...
    call matern52_gradient(distance, lv, ll, y_bar, expected_distance_bar, &
    ...
    call matern52_gradient(plus(1), plus(2), plus(3), y_bar, gradient_plus(1), &
        gradient_plus(2), gradient_plus(3))
    call matern52_gradient(minus(1), minus(2), minus(3), y_bar, gradient_minus(1), &
    ...
    if (failures > 0) then
        write (error_unit, '(a,i0)') 'FAIL FortSym Matern52 cases: ', failures
        error stop 1
    end if
    write (*, '(a)') 'PASS FortSym-generated Matern52 independent oracles'

contains

    real(dp) function matern52_value(d, log_variance, log_lengthscale) result(value)
        real(dp), intent(in) :: d, log_variance, log_lengthscale
        real(dp) :: r

        r = kappa*d*exp(-log_lengthscale)
        value = exp(log_variance)*(1.0_dp + r + r*r/3.0_dp)*exp(-r)
    end function matern52_value

    subroutine matern52_gradient(d, log_variance, log_lengthscale, cotangent, &
        ...
        exp_minus_r = exp(-r)
        dvalue_dr = -exp_lv*r*(1.0_dp + r)*exp_minus_r/3.0_dp
        d_bar = cotangent*dvalue_dr*kappa*exp(-log_lengthscale)
        lv_bar_out = cotangent*exp_lv*(1.0_dp + r + r*r/3.0_dp)*exp_minus_r
        ll_bar_out = cotangent*dvalue_dr*(-r)
    end subroutine matern52_gradient
    ...
    subroutine check(condition, name, failures)
        ...
        if (.not. condition) then
            failures = failures + 1
            write (error_unit, '(a)') '  FAIL [fortsym-matern52] '//name
        end if
    end subroutine check

end program test_fortsym_matern52
