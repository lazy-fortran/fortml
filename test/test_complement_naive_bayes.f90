program test_complement_naive_bayes
    !! Independent analytic and differential checks for ComplementNB.
    ...
    type(complement_naive_bayes_t) :: model, norm_model, weighted_model, unfitted
    ...
    real(dp) :: expected_prior(3), expected_weight(3, 3), normalizer, lhs, rhs, step
    ...
    expected_q = reshape([ &
        2.0_dp/5.0_dp, 3.0_dp/10.0_dp, 3.0_dp/10.0_dp, &
        1.0_dp/3.0_dp, 1.0_dp/3.0_dp, 1.0_dp/3.0_dp, &
        1.0_dp/4.0_dp, 3.0_dp/8.0_dp, 3.0_dp/8.0_dp], shape(expected_q))
    call check(maxval(abs(q - expected_q)) < 2.0e-14_dp, &
        "analytic complement probability oracle", failures)
    ...
    expected_weight = -log(expected_q)
    call check(maxval(abs(model%feature_weights() - expected_weight)) < 2.0e-14_dp, &
        "analytic complement weight oracle", failures)
    ...
    do i = 1, 3
        ...
        do c = 1, 3
            do j = 1, 3
                expected_log(c) = expected_log(c) + query(i, j)*expected_weight(j, c)
            end do
        end do
    ...
    call norm_model%fit(x, labels, status, alpha=1.0_dp, norm=.true.)
    call check(status_ok(status) .and. norm_model%norm_enabled() .and. &
        maxval(abs(sum(norm_model%feature_weights(), dim=1) - 1.0_dp)) < 2.0e-14_dp, &
        "second weight normalization", failures)
    call norm_model%predict_log_proba_jvp(query, query_dot, log_probabilities, &
        log_dot, status)
    call norm_model%predict_log_proba(query + step*query_dot, log_plus, status)
    call norm_model%predict_log_proba(query - step*query_dot, log_minus, status)
    call check(status_ok(status) .and. maxval(abs(log_dot - &
        (log_plus - log_minus)/(2.0_dp*step))) < 3.0e-7_dp, &
        "normalized-weight input JVP finite difference", failures)
    ...
    counts = weighted_model%feature_counts()
    expected_counts = reshape([0.0_dp, 2.0_dp, 2.0_dp, 2.0_dp, 2.0_dp, 2.0_dp, &
        6.0_dp, 3.0_dp, 3.0_dp], shape(expected_counts))
    expected_prior = [4.0_dp, 4.0_dp, 6.0_dp]/14.0_dp
    call check(status_ok(status) .and. maxval(abs(counts - expected_counts)) < 2.0e-14_dp .and. &
    ...
    if (failures > 0) then
        write (error_unit, '(a,i0)') "FAIL ComplementNB cases: ", failures
        error stop 1
    end if
    write (*, '(a)') "PASS Complement Naive Bayes independent analytic oracles"
...
contains
    ...
    subroutine check(condition, name, failures)
        ...
        if (.not. condition) then
            failures = failures + 1
            write (error_unit, '(a)') "  FAIL [complement-nb] "//name
        end if
    end subroutine check

end program test_complement_naive_bayes
