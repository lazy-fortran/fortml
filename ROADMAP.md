...
| XGBoost/LightGBM | Exact and bounded-histogram depth-limited XGBoost-style squared/logistic/Poisson/squared-log (RMSLE)/Huber/quantile/absolute Newton trees, weighted binary/OVR multiclass staged predictions, bounded `rank:pairwise`, margins, gain/weight/cover diagnostics, per-feature monotonic constraints, plus a separately named `lightgbm_t` weighted regression/binary-logistic path with shared weighted-quantile cuts and deterministic globally best-leaf growth up to `num_leaves` | Tweedie, categorical splits, DART, GOSS/EFB, interaction constraints, distributed training, staged/warm-start LightGBM state, and resident GPU histograms |
