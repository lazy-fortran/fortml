...
The matching `fortml-bench` revision is `5ed3c53`.

The next parity wave adds three production slices. MLP loss scaling now has
allocation-free scale, finite-check, and unscale products integrated into the
trainer before optimizer commit. Independent multilabel Laplace-GP heads now
support FortOpt L-BFGS-B over one packed kernel block per label. The LightGBM
multiclass adapter now exposes stable log probabilities and packed leaf
parameter products. The combined source gate reaches 662 modules, 962 build
units, and 293/293 tests with lint clean. The canonical source feature tip is
`25cc887`. The corresponding benchmark records use independent oracle errors
of zero for the loss-scale lane, `1.03e-11` for the GP HPO products, and
`1.74e-18` for the LightGBM derivative products.
...
state, exact fixed-fit prediction/input/parameter JVP/VJP products, and a
typed CUDA refusal. `test_weighted_ols`, `app/fortml_bench_weighted_ols`, and
the dedicated `fortml-bench` release record provide independent normal-equation,
finite-difference, adjoint, malformed-weight, and device-boundary evidence.
...
fixed-state input/parameter JVP/VJP products, and explicit CPU/CUDA capability
rows. `test_rbf_svm_multiclass` and the independent SciPy/NumPy lane in
`fortml-bench/results/RBF_SVM_MULTICLASS.md` agree on scores, probabilities,
labels, and packed-state behavior; fit active-set derivatives, sparse/cache
execution, and resident CUDA batching remain open.
...
| --- | --- | --- |
| GNU Fortran | `fo` | Static build, all 282 behavioral tests, and lint passed at the current integrated FortML/FortAD-main revisions (636 modules, 947 build units). The compiler still emits non-fatal array-temporary warnings; see [`verification/fortml-gfortran.txt`](verification/fortml-gfortran.txt). |
| NVIDIA HPC SDK | `FO_FC=nvfortran fo` | Static and lint checks passed in the recorded older compiler lane. The checked-in NVIDIA log predates the current 282-test GNU run. See [`verification/fortml-nvfortran.txt`](verification/fortml-nvfortran.txt). |
...
revision `c7d7e90` (including shared-kernel multilabel GP optimization, multiclass RBF SVM, fixed-state Student-t likelihood, analytic ReLU NNGP, PINN structure-aware GP, scalar Lagrangian MLP, deterministic linear SGD,
...
`26362066948d52ebb087aae2b3c668476a40f3c1`, FortFront at
`6c27ca86a52066e46d318633febca52d41c106b6` (not a direct fpm dependency), and FortNum at
...
shape conversions. They are isolated to array construction; all 282 behavioral
...
### 2026-08-09 bounded parity closure slices

Deterministic MLP SGD/Nesterov accumulation now has explicit contiguous
`microbatch_size` and `accumulation_steps` state. Row-mass reductions are
shared by value, JVP, VJP, affine HVP, and the FortOpt L-BFGS-B callback.
Malformed layouts are transactional, and selected CUDA requests return a typed
refusal. The independent release row is
`fortml-bench/results/sgd_momentum_hypergradient_accumulation.csv`, which pins
FortML `0e147b4` and FortML-bench `ed64fe8` and reports a maximum CPU oracle
error of `1.79e-8`.

Weighted random-forest regression now covers scalar and multi-output targets,
seeded bootstrap inclusion state, staged prefix predictions, split-frequency
feature importances, fixed-state input JVP/VJP products, transactional fit,
and an exact split-boundary refusal. The independent NumPy weighted-CART
replay reports zero prediction error in
`fortml-bench/results/random_forest_regression.csv`, pinned to FortML
`8baedc8` and FortML-bench `84cbcda`; resident CUDA remains a typed refusal.

The robust Poisson Laplace-GP path now exposes normalized likelihood
value/gradient/JVP/VJP/HVP products, fixed-mode latent posterior and query
products, transactional latent setters, and a bounded FortOpt L-BFGS-B
objective. `fortml-bench/results/robust_gp_poisson_products.csv` pins FortML
`13c352b` and FortML-bench `19b95c1`, with posterior HVP error `1.23e-12`,
...
`065609d586a44b3becd2f790356642fceaca2c3e` and FortMC
...
`26362066948d52ebb087aae2b3c668476a40f3c1`, FortFront
`6c27ca86a52066e46d318633febca52d41c106b6`, FortSym
`26250ce24e3890711343a2f18d5feae2be832508`, FortOpt
`883aa7ee0a3f2b3a508d0309b907adccc30a0f29`, and FortNum
`7ced2f7`, all checked against their corresponding remote branches on
...
`065609d586a44b3becd2f790356642fceaca2c3e`, both on their `main` branches. The
...
and remains limited to the pushing arm. Against the current 065609d tip,
...
| --- | --- | --- |
| scikit-learn linear/GLM | Dense linear regression, weighted ridge/lasso/elastic-net, positive-constrained, Huber and quantile regression, Poisson/Gamma GLM, logistic/softmax, bounded logistic and MLP L-BFGS-B | ARD/Tweedie, SGD estimators, solver parity, calibration, complete multioutput and partial-fit contracts |
...
| scikit-learn trees/ensembles | Stumps, weighted CART with deterministic NaN routing, seeded weighted scalar/multi-output random-forest regression with staged prefixes and split-frequency importances, binary and multiclass SAMME and SAMME.R AdaBoost over weighted CART, squared boosting, exact and histogram XGBoost-style second-order squared/binary-logistic/Poisson/Tweedie/squared-log/absolute lanes, bounded `rank:pairwise`, per-feature monotonic and interaction-group constraints, bounded ordered-gradient integer categorical partitions with explicit cardinality refusal, transactional XGBoost warm-start continuation, bounded LightGBM-style weighted leaf-wise regression/binary growth with staged margins/predictions, additive contributions, fitted-prefix slicing, versioned persistence, matched-option warm starts including validation-aware continuation, and seeded XGBoost/LightGBM DART/dropout tree normalisation | Extra forests, bagging, categorical policies beyond ordered partitions, XGBoost EFB, distributed growth, and resident GPU histograms |
...
| Multiclass RBF SVM | `fortml_rbf_svm_multiclass` now provides a transactional deterministic one-vs-rest wrapper over finite-basis `rbf_svm_classifier_t`: arbitrary sorted integer labels, normalized sigmoid-margin simplex probabilities, packed per-class `[coefficients,intercept,log(gamma)]` metadata, fixed-state input/parameter probability JVP/VJP products, CPU dispatch, and explicit CUDA refusals are covered by independent Fortran and NumPy-oracle benchmark lanes. | Fit active-set/hyperparameter-through-fit derivatives, sparse/kernel-cache execution, and resident CUDA batching remain open. |
| OVO RBF SVM | `fortml_ovo_rbf_svm_classifier` now provides deterministic sorted-label one-vs-one finite-basis RBF-SVM fits, pair metadata, normalized pairwise-vote probabilities, original-label prediction, pair-specific packed parameter offsets, decision/probability input and parameter JVP/VJP products, transactional malformed-fit behavior, and explicit CPU/CUDA refusal semantics. Independent Fortran finite-difference/adjoint tests and a NumPy/SciPy oracle benchmark are recorded in `fortml-bench/results/OVO_RBF_SVM.md`. | Fit-state and squared-hinge active-set derivatives, sparse/kernel-cache execution, pairwise coupling beyond normalized votes, and resident CUDA batching remain open. |
...
- [x] Weighted scalar and multi-output random-forest regression with seeded
  bootstrap inclusion state, staged prefix predictions, split-frequency
  importances, transactional fit, fixed-state input JVP/VJP products with
  split-boundary refusals, typed CUDA refusal, and an independent NumPy
  weighted-CART replay in `RANDOM_FOREST_REGRESSION.md`.
...
- [x] Add a bounded Laplace robust/count GP path in `robust_gp_t`. Poisson
  ...
  The Poisson extension adds normalized likelihood value/gradient/JVP/VJP/HVP
  products, fixed-mode posterior and query products, transactional latent
  setters, a bounded FortOpt L-BFGS-B objective, and the independent
...
- [x] Add the shared-kernel multilabel GP outer-HPO slice. `shared_parameters`
  ...
  open.
- [x] Add independent multilabel Laplace-GP hyperparameter optimization.
  `fixed_state_independent_value_gradient` packs one kernel-log block per
  label, constructs all candidate factors transactionally, and supplies exact
  value/gradient/JVP/VJP callbacks to FortOpt L-BFGS-B through
  `optimize_independent_lbfgsb`. The independent objective decreases on the
  weighted two-label fixture with a `1.03e-11` maximum NumPy product error.
  The per-label fixed-state mode and resident CUDA reduction remain explicit
  boundaries.
- [x] Add stable multilabel Laplace-GP log probabilities and product dispatch.
  `predict_log_proba` evaluates each independent label directly in log space,
  while input, per-label packed-parameter, and shared-kernel JVP/VJP methods
  expose CPU execution and typed CUDA refusals without mutating output buffers.
  `test_gp_multilabel_classification` and
  `fortml-bench/results/GP_MULTILABEL_LOG_PROBA.md` provide the independent
  NumPy log-probability, derivative, and round-trip oracle.
...
The source inventory is dated 2026-08-09.
...
- [x] Complete multiclass XGBoost log-probability products. The sorted-label
  one-vs-rest adapter evaluates stable class log probabilities with log-sum-exp
  normalization, exposes input and packed leaf-coordinate JVP/VJP products
  over every child tree, and provides explicit CPU/CUDA dispatch and metadata
  aliases. The independent NumPy release lane is
  `fortml-bench/results/XGBOOST_MULTICLASS_LOG_PROBA.md`.
...
- [x] Add stable LightGBM multiclass log-probability products. The sorted-label
  OVR adapter evaluates log-sigmoid links with log-sum-exp normalization and
  exposes input plus packed leaf-coordinate JVP/VJP products, aliases, and
  explicit CPU/CUDA dispatch. The independent release lane records a maximum
  derivative error of `1.74e-18` and a typed CUDA refusal.
...
- [x] Add fixed-hyperparameter weighted Bayesian ridge regression with dense
  vector and multi-output posterior mean/precision/covariance, nonnegative
  sample weights, explicit alpha/lambda and log-evidence metadata, and exact
  fixed-state coefficient/input JVP/VJP products. The independent closed-form
  posterior oracle, malformed/all-zero-weight refusals, and typed CUDA refusal
  are covered by `test_bayesian_ridge` and
  `fortml_bench_bayesian_ridge`; evidence maximisation, ARD, and
  derivative-through-fit remain open.
- [x] Add weighted OLS with optional intercept, finite nonnegative sample
  weights, deterministic multi-output SVD fitting, packed coefficient state,
  fixed-fit prediction/input/parameter JVP/VJP products, independent normal-
  equation and adjoint tests, and an explicit CUDA refusal. The release app
  and NumPy oracle are recorded in `fortml-bench`.
- [x] Add positive-constrained weighted multi-output linear regression with a
  deterministic projected-gradient solver, optional unconstrained or
  nonnegative intercept, transactional convergence/refit and packed-parameter
  updates, fixed-state prediction/input/parameter JVP/VJP products, an
  independent behavioral oracle, and an explicit CPU/CUDA capability boundary.
  The release smoke app and NumPy correctness-gated benchmark are recorded in
  `fortml-bench`; active-set and fit-through derivatives remain explicit
  boundaries.
- [ ] Add ARD, Theil-Sen, RANSAC, and Tweedie regression. Solvers expose
...
- [x] Add the exact affine RMSprop outer HVP. The one-layer all-linear branch
  differentiates the trajectory value/gradient product a second time through
  centered and uncentered running statistics, momentum, and validation loss.
  Nonlinear and multilayer networks return a typed third-derivative refusal.
  `fortml-bench/results/RMSPROP_HYPERGRADIENT_HVP.md` records the independent
  central-difference value, gradient, JVP, and HVP oracle.
...
- [x] Add transactional MLP loss-scale gradient products. The trainer applies
  scale, finite-check, unscale, and overflow backoff before any optimizer
  mutation. `scale_gradient`, `unscale_gradient`, and
  `scaled_gradient_finite` are allocation-free and preserve output/state on a
...
- [x] Add a resident no-autodiff CUDA dense MLP-chain inference plan. The
  `cuda_mlp_chain_plan_t`/C ABI keeps a validated sequence of dense layers,
  topology, reusable activations, and preactivations on one selected device;
  prediction plus packed all-layer input/parameter JVP and VJP products stay
  device-side between explicit batch/product transfers. Workspace reuse,
  transfer/resident-byte counters, an independent three-layer CPU oracle, and
  the ordinary-build typed stub are covered by `test/run_cuda_mlp_chain.sh` and
  `test_cuda_mlp_chain_api`. This does not claim resident training, HVP,
  mixed-precision, or a device-resident FortAD/FortSym graph.
...
`26362066948d52ebb087aae2b3c668476a40f3c1` and `fortsym` `main` at
...
- [`RBF_SVM.md`](../fortml-bench/results/RBF_SVM.md), backed by `rbf_svm.csv`
  ...
  contract.
- [`OVO_RBF_SVM.md`](../fortml-bench/results/OVO_RBF_SVM.md), backed by
  `ovo_rbf_svm.csv` for sorted pair metadata, normalized pairwise-vote
  probabilities, decision/probability derivative products, and the explicit
  CUDA refusal.
