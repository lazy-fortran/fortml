program fortml_bench_mlp_optimizer_group_hypergradient
    !! Release workload for fixed full-batch optimizer-group products.
    ...
    if (.not. status_ok(status)) error stop "optimizer-group benchmark weight setup failed"
    call bias_group%initialize("bias", 2, 2, 1.25_dp, status)
    if (.not. status_ok(status)) error stop "optimizer-group benchmark bias setup failed"
    ...
    if (.not. status_ok(status)) error stop "optimizer-group benchmark model initialization failed"
    call model%set_parameters([0.21_dp, 0.06_dp], status)
    if (.not. status_ok(status)) error stop "optimizer-group benchmark parameter setup failed"
    call objective%initialize(model, train_x, train_target, validation_x, &
        validation_target, options, status)
    if (.not. status_ok(status)) error stop "optimizer-group benchmark setup failed"
    parameters = objective%parameters()
    call objective%value_gradient(parameters, value, gradient, status)
    if (.not. status_ok(status)) error stop "optimizer-group benchmark product failed"
    ...
    do repetition = 1, repetitions
        call objective%value_gradient(parameters, value, gradient, status)
        if (.not. status_ok(status)) error stop "optimizer-group benchmark timing failed"
    end do
    call system_clock(clock_end)
    elapsed = real(clock_end-clock_start, dp)/real(clock_rate, dp) &
        /real(repetitions, dp)
    write (*, '(a,es24.16)') "mlp_optimizer_group_hypergradient_value_gradient,", elapsed
...
end program fortml_bench_mlp_optimizer_group_hypergradient
