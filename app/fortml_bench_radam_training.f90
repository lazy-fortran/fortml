program fortml_bench_radam_training
    !! Release workload for RAdam state and MLP trainer integration.
    use, intrinsic :: iso_fortran_env, only: dp => real64, int64
    use fortml_radam, only: radam_t
    use fortml_mlp, only: mlp_t, MLP_LINEAR
    use fortml_mlp_training, only: mlp_training_options_t, mlp_training_state_t, &
        mlp_train, MLP_OPTIMIZER_RADAM
    ...
    if (.not. status_ok(status)) error stop "RAdam benchmark initialization failed"
    do step = 1, steps
        gradient = parameter - target
        call optimizer%step(parameter, gradient, status)
        if (.not. status_ok(status)) error stop "RAdam benchmark reference failed"
    end do
    ...
    do repetition = 1, repetitions
        ...
        call optimizer%initialize(n_parameters, status, learning_rate=1.0e-2_dp, &
            beta1=0.9_dp, beta2=0.99_dp, epsilon=1.0e-8_dp)
        if (.not. status_ok(status)) error stop "RAdam benchmark timing initialization failed"
        do step = 1, steps
            gradient = parameter - target
            call optimizer%step(parameter, gradient, status)
            if (.not. status_ok(status)) error stop "RAdam benchmark timing failed"
        end do
    end do
    ...
    write (*, '(a,i0,a,i0,a,es24.16,a,es24.16)') &
        "radam_training,", n_parameters, ",", steps, ",", final_norm, ",", elapsed
    ...
    if (.not. status_ok(status)) error stop "MLP RAdam benchmark reference failed"
    ...
    do repetition = 1, mlp_repetitions
        call model%set_parameters([0.0_dp, 0.0_dp], status)
        call mlp_train(model, x, mlp_target, status, options, state)
        if (.not. status_ok(status)) error stop "MLP RAdam benchmark timing failed"
    end do
    call system_clock(clock_end)
    ...
    write (*, '(a,i0,a,i0,a,es24.16,a,es24.16,a,es24.16)') &
        "radam_mlp,", n_samples, ",", n_epochs, ",", mlp_loss, ",", &
        mlp_elapsed, ",", sqrt(sum(mlp_theta*mlp_theta))
end program fortml_bench_radam_training
