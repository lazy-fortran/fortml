program fortml_bench_mlp_schedule_hvp
    !! Complete-array release workload for affine scheduled MLP outer HVPs.
    ...
    integer, parameter :: n_train = 32, n_validation = 16, repetitions = 32
    integer, parameter :: steps = 6
    ...
    real(dp) :: parameters(4), direction(4), gradient(4), hvp(4)
    real(dp) :: value, tangent, elapsed, elapsed_hvp
    ...
    options%l2 = l2
    options%schedule = make_mlp_schedule_warmup_cosine(2, 10, 0.15_dp)
    call model%initialize([1, 1], status, hidden_activation=MLP_LINEAR, &
        output_activation=MLP_LINEAR, initialization_seed=23)
    if (.not. status_ok(status)) error stop "scheduled HVP benchmark initialization failed"
    call model%set_parameters([0.15_dp, -0.1_dp], status)
    if (.not. status_ok(status)) error stop "scheduled HVP benchmark parameter setup failed"
    call objective%initialize(model, train_x, train_target, validation_x, &
        validation_target, options, status)
    if (.not. status_ok(status)) error stop "scheduled HVP benchmark setup failed"
    parameters = objective%parameters()
    direction = [0.31_dp, -0.27_dp, 0.18_dp, -0.22_dp]
    call objective%value_gradient(parameters, value, gradient, status)
    if (.not. status_ok(status)) error stop "scheduled HVP benchmark value failed"
    call objective%jvp(parameters, direction, value, tangent, status)
    if (.not. status_ok(status)) error stop "scheduled HVP benchmark JVP failed"
    call objective%hvp(parameters, direction, hvp, status)
    if (.not. status_ok(status)) error stop "scheduled HVP benchmark HVP failed"

    oracle_unit = -1
    call get_environment_variable("FORTML_BENCH_MLP_SCHEDULE_HVP_ORACLE", &
    ...
    do repetition = 1, repetitions
        call objective%value_gradient(parameters, value, gradient, status)
        if (.not. status_ok(status)) error stop "scheduled HVP benchmark value timing failed"
    end do
    ...
    do repetition = 1, repetitions
        call objective%hvp(parameters, direction, hvp, status)
        if (.not. status_ok(status)) error stop "scheduled HVP benchmark HVP timing failed"
    end do
    call system_clock(clock_end)
    elapsed_hvp = real(clock_end-clock_start, dp)/real(clock_rate, dp)/real(repetitions, dp)
    write (*, '(a,es24.16)') "mlp_schedule_hvp_value_gradient,", elapsed
    write (*, '(a,es24.16)') "mlp_schedule_hvp_hvp,", elapsed_hvp

contains

    subroutine make_fixture(x_train, y_train, x_validation, y_validation)
        real(dp), intent(out) :: x_train(:, :), y_train(:, :)
        real(dp), intent(out) :: x_validation(:, :), y_validation(:, :)
        integer :: i

        do i = 1, size(x_train, 1)
            x_train(i, 1) = -2.0_dp + 4.0_dp*real(i-1, dp)/real(size(x_train, 1)-1, dp)
            y_train(i, 1) = 0.7_dp*x_train(i, 1) - 0.2_dp
        end do
        do i = 1, size(x_validation, 1)
            x_validation(i, 1) = -1.8_dp + 3.6_dp*real(i-1, dp)/ &
                real(size(x_validation, 1)-1, dp)
            y_validation(i, 1) = 0.7_dp*x_validation(i, 1) - 0.2_dp
        end do
...
end program fortml_bench_mlp_schedule_hvp
