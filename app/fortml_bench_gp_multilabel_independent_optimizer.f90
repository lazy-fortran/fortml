program fortml_bench_gp_multilabel_independent_optimizer
    !! Release probe for independent multilabel GP fixed-state HPO.
    ...
    use fortml_gp_multilabel_classification, only: &
        ...
        gp_multilabel_classification_state_t, gp_multilabel_training_objective_t, &
        gp_multilabel_lbfgsb_options_t, gp_multilabel_lbfgsb_result_t, &
    ...
    real(dp) :: x(10, 1), parameters(4), direction(4), gradient(4)
    real(dp) :: objective_value, objective_value_plus, objective_value_minus
    real(dp) :: objective_tangent, objective_vjp(4), fd_plus, fd_minus, h
    real(dp), allocatable :: final_parameters(:)
    ...
    call model%fit(x, indicators, kernel, status, fit_options, fit_state, &
        sample_weight=[1.0_dp, 0.9_dp, 1.1_dp, 1.0_dp, 0.8_dp, 1.2_dp, 1.0_dp, &
        1.1_dp, 0.9_dp, 1.0_dp])
    if (.not. status_ok(status)) error stop 1

    parameters = model%parameters()
    direction = [0.17_dp, -0.11_dp, -0.13_dp, 0.07_dp]
    call model%fixed_state_independent_value_gradient(parameters, objective_value, &
        gradient, status)
    if (.not. status_ok(status)) error stop 2
    h = 1.0e-5_dp
    call model%fixed_state_independent_value_gradient(parameters + h*direction, &
        objective_value_plus, objective_vjp, status)
    if (.not. status_ok(status)) error stop 3
    fd_plus = objective_value_plus
    call model%fixed_state_independent_value_gradient(parameters - h*direction, &
        objective_value_minus, objective_vjp, status)
    if (.not. status_ok(status)) error stop 4
    fd_minus = objective_value_minus
    call model%fixed_state_independent_value_gradient(parameters, objective_value_plus, &
        objective_vjp, status)
    if (.not. status_ok(status)) error stop 5
    call objective%initialize_independent(model, status)
    ...
    optimizer_options%max_line_search = 80
    optimizer_options%lower_bound = -1.0_dp
    optimizer_options%upper_bound = 1.0_dp
    optimizer_options%gradient_tolerance = 5.0e-3_dp
    optimizer_options%objective_tolerance = 1.0e-6_dp
    optimizer_options%step_tolerance = 1.0e-6_dp
    call gp_multilabel_optimize_independent_lbfgsb(model, optimizer_options, &
        optimizer_result, status)
    status_code = status%code
    final_parameters = model%parameters()

    write (*, '(a,es24.16,a,es24.16,a,es24.16,a,es24.16)') &
        'gp_multilabel_independent_parameters,', parameters(1), ',', parameters(2), &
        ',', parameters(3), ',', parameters(4)
    write (*, '(a,es24.16)') 'gp_multilabel_independent_objective,', objective_value
    write (*, '(a,es24.16,a,es24.16,a,es24.16,a,es24.16)') &
        'gp_multilabel_independent_gradient,', gradient(1), ',', gradient(2), ',', &
        gradient(3), ',', gradient(4)
    write (*, '(a,es24.16)') 'gp_multilabel_independent_jvp,', objective_tangent
    write (*, '(a,es24.16,a,es24.16,a,es24.16,a,es24.16)') &
        'gp_multilabel_independent_vjp,', objective_vjp(1), ',', objective_vjp(2), ',', &
        objective_vjp(3), ',', objective_vjp(4)
    write (*, '(a,es24.16,a,es24.16)') 'gp_multilabel_independent_fd_values,', &
        fd_plus, ',', fd_minus
    write (*, '(a,i0,a,l1,a,i0,a,es24.16,a,es24.16)') &
        'gp_multilabel_independent_result,', status_code, ',', optimizer_result%converged, &
        ',', optimizer_result%iterations, ',', optimizer_result%objective, ',', &
        optimizer_result%gradient_norm
    write (*, '(a,i0)') 'gp_multilabel_independent_labels,', size(indicators, 2)
    do i = 1, size(parameters)
        write (*, '(a,i0,a,es24.16)') 'gp_multilabel_independent_final_parameter,', i, ',', &
            final_parameters(i)
    end do
    write (*, '(a,i0)') 'gp_multilabel_independent_cuda,', &
        merge(1, 0, model%device_supported(FORTML_DEVICE_CUDA))
end program fortml_bench_gp_multilabel_independent_optimizer
