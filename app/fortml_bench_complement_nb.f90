program fortml_bench_complement_nb
    !! Release workload for differentiable Complement Naive Bayes.
    !!
    !! The Python harness reconstructs complement counts, positive weights,
    !! stable probabilities, and the input JVP independently.  This app emits
    !! complete output arrays when FORTML_BENCH_COMPLEMENT_NB_ORACLE is set,
    !! then reports release timings.
    ...
    integer :: j, k
    real(dp) :: fit_elapsed, prediction_elapsed, jvp_elapsed
    integer(int64) :: clock_start, clock_end, clock_rate
    type(complement_naive_bayes_t) :: model
    ...
    if (.not. status_ok(status)) error stop "ComplementNB benchmark fit failed"
    call model%predict_log_proba(x, log_probabilities, status)
    if (.not. status_ok(status)) error stop "ComplementNB benchmark log prediction failed"
    call model%predict_proba(x, probabilities, status)
    if (.not. status_ok(status)) error stop "ComplementNB benchmark prediction failed"
    call model%predict(x, predicted, status)
    if (.not. status_ok(status)) error stop "ComplementNB benchmark labels failed"
    call model%predict_log_proba_jvp(x, x_dot, log_probabilities, &
        log_probabilities_dot, status)
    if (.not. status_ok(status)) error stop "ComplementNB benchmark JVP failed"
    if (maxval(abs(sum(probabilities, dim=2) - 1.0_dp)) > 2.0e-14_dp) then
        error stop "ComplementNB benchmark probability oracle failed"
    end if

    oracle_unit = -1
    call get_environment_variable("FORTML_BENCH_COMPLEMENT_NB_ORACLE", oracle_path, &
    ...
    do k = 1, fit_repetitions
        call model%fit(x, labels, status, alpha=alpha)
        if (.not. status_ok(status)) error stop "ComplementNB timed fit failed"
    end do
    ...
    do k = 1, prediction_repetitions
        call model%predict_log_proba(x, log_probabilities, status)
        if (.not. status_ok(status)) error stop "ComplementNB timed prediction failed"
    end do
    call system_clock(clock_end)
    ...
    do k = 1, jvp_repetitions
        call model%predict_log_proba_jvp(x, x_dot, log_probabilities, &
            log_probabilities_dot, status)
        if (.not. status_ok(status)) error stop "ComplementNB timed JVP failed"
    end do
    call system_clock(clock_end)
    ...
    write (*, '(a,i0,a,i0,a,i0,a,es24.16)') &
        "complement_nb_fit,", n_samples, ",", n_features, ",", n_classes, ",", &
        fit_elapsed
    write (*, '(a,i0,a,i0,a,i0,a,es24.16)') &
        "complement_nb_predict,", n_samples, ",", n_features, ",", n_classes, ",", &
        prediction_elapsed
    write (*, '(a,i0,a,i0,a,i0,a,es24.16)') &
        "complement_nb_jvp,", n_samples, ",", n_features, ",", n_classes, ",", &
        jvp_elapsed
...
end program fortml_bench_complement_nb
