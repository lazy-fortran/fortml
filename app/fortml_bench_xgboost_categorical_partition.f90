program fortml_bench_xgboost_categorical_partition
    !! Release workload for exhaustive small-cardinality categorical splits.
    ...
    do repetition = 1, repetitions
        ...
        if (.not. status_ok(status)) error stop "categorical partition fit failed"
    end do
    call system_clock(clock_end)
    elapsed_fit = real(clock_end - clock_start, dp)/real(clock_rate, dp)/real(repetitions, dp)
    call model%predict(x, prediction, status)
    if (.not. status_ok(status)) error stop "categorical partition prediction failed"
    write (*, '(a,i0,a,i0,a,i0,a,es24.16,a,i0,a,es24.16)') &
        "xgb_categorical_partition_fit,", n_samples, ",", n_features, ",", &
        n_estimators, ",", elapsed_fit, ",", model%tree_node_count(1), ",", &
    ...
    call model%predict(x, prediction, status)
    write (*, '(a,*(1x,es24.16))') "xgb_categorical_partition_values", prediction
    write (*, '(a,es24.16)') "xgb_categorical_partition_predict_seconds ", elapsed_predict
end program fortml_bench_xgboost_categorical_partition
