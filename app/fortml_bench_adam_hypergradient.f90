program fortml_bench_adam_hypergradient
    !! Release workload for the four-parameter coupled-L2 Adam objective.
    ...
    integer, parameter :: steps = 4, parameter_count = 4
    real(dp), parameter :: learning_rate = 0.12_dp, l2 = 0.07_dp
    real(dp), parameter :: beta1 = 0.82_dp, beta2 = 0.91_dp, epsilon = 0.03_dp
    ...
    type(mlp_adam_hypergradient_options_t) :: options
    ...
    if (.not. status_ok(status)) error stop "Adam hypergradient benchmark initialization failed"
    call model%set_parameters([0.15_dp, -0.1_dp], status)
    if (.not. status_ok(status)) error stop "Adam hypergradient benchmark parameter setup failed"
    call objective%initialize(model, train_x, train_target, validation_x, &
        validation_target, options, status)
    if (.not. status_ok(status)) error stop "Adam hypergradient benchmark setup failed"
    parameters = objective%parameters()
    call objective%value_gradient(parameters, value, gradient, status)
    if (.not. status_ok(status)) error stop "Adam hypergradient value/gradient failed"
    direction = [0.31_dp, -0.27_dp, 0.13_dp, -0.22_dp]
    call objective%jvp(parameters, direction, value, tangent, status)
    if (.not. status_ok(status)) error stop "Adam hypergradient JVP failed"

    oracle_unit = -1
    call get_environment_variable("FORTML_BENCH_ADAM_HYPERGRADIENT_ORACLE", oracle_path, &
    ...
    do repetition = 1, repetitions
        call objective%value_gradient(parameters, value, gradient, status)
        if (.not. status_ok(status)) error stop "Adam hypergradient timing failed"
    end do
    call system_clock(clock_end)
    elapsed = real(clock_end-clock_start, dp)/real(clock_rate, dp) &
        /real(repetitions, dp)
    write (*, '(a,es24.16)') "mlp_adam_hypergradient_value_gradient,", elapsed
...
end program fortml_bench_adam_hypergradient
