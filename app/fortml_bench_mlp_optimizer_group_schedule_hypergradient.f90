program fortml_bench_mlp_optimizer_group_schedule_hypergradient
    !! Release workload for scheduled optimizer-group products.
    ...
    train_x(:, 1) = [-1.5_dp, -0.8_dp, -0.1_dp, 0.6_dp, 1.4_dp, 2.0_dp]
    train_target(:, 1) = 0.75_dp*train_x(:, 1) - 0.2_dp
    validation_x(:, 1) = [-1.2_dp, 0.25_dp, 1.7_dp]
    validation_target(:, 1) = 0.75_dp*validation_x(:, 1) - 0.2_dp
    options%steps = 4
    ...
    options%lower_logit_min_fraction = -4.0_dp
    options%upper_logit_min_fraction = 4.0_dp
    options%lower_logit_decay_factor = -4.0_dp
    options%upper_logit_decay_factor = 4.0_dp
    options%lower_log_multiplier = -4.0_dp
    options%upper_log_multiplier = 4.0_dp
    call weight_group%initialize("weight", 1, 1, 0.65_dp, status)
    if (.not. status_ok(status)) error stop "scheduled group weight setup failed"
    call bias_group%initialize("bias", 2, 2, 1.25_dp, status)
    if (.not. status_ok(status)) error stop "scheduled group bias setup failed"
    ...
    if (.not. status_ok(status)) error stop "scheduled group model initialization failed"
    call model%set_parameters([0.21_dp, 0.06_dp], status)
    if (.not. status_ok(status)) error stop "scheduled group parameter setup failed"
    call objective%initialize(model, train_x, train_target, validation_x, &
        validation_target, options, status)
    if (.not. status_ok(status)) error stop "scheduled group objective setup failed"
    ...
    oracle_unit = -1
    call get_environment_variable("FORTML_BENCH_OPTIMIZER_GROUP_SCHEDULE_ORACLE", &
    ...
    if (environment_status == 0 .and. len_trim(oracle_path) > 0) then
        ...
        do repetition = 1, size(parameters)
            write (oracle_unit, '(a,i0,a,es26.17e3)') "gradient,", repetition, ",", gradient(repetition)
        end do
    ...
    do repetition = 1, repetitions
        call objective%value_gradient(parameters, value, gradient, status)
        if (.not. status_ok(status)) error stop "scheduled group timing failed"
    end do
    call system_clock(clock_end)
    elapsed = real(clock_end-clock_start, dp)/real(clock_rate, dp)/real(repetitions, dp)
    write (*, '(a,es24.16)') "mlp_optimizer_group_schedule_hypergradient_value_gradient,", elapsed
...
end program fortml_bench_mlp_optimizer_group_schedule_hypergradient
