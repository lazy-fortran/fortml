program fortml_bench_adagrad_hypergradient
    ...
    if (.not. status_ok(status)) error stop "Adagrad hypergradient benchmark initialization failed"
    call model%set_parameters([0.15_dp, -0.1_dp], status)
    if (.not. status_ok(status)) error stop "Adagrad hypergradient parameter setup failed"
...
end program fortml_bench_adagrad_hypergradient
