program fortml_bench_amsgrad_hypergradient
    !! Release workload for the fixed full-batch AMSGrad hypergradient.
    ...
    options%lower_logit_beta1 = -5.0_dp
    options%upper_logit_beta1 = 5.0_dp
    options%lower_logit_beta2 = -5.0_dp
    options%upper_logit_beta2 = 5.0_dp
    options%lower_log_epsilon = -5.0_dp
    options%upper_log_epsilon = 0.0_dp

    call model%initialize([1, 1], status, initialization_seed=23)
    if (.not. status_ok(status)) error stop "AMSGrad hypergradient benchmark initialization failed"
    call model%set_parameters([0.15_dp, -0.1_dp], status)
    if (.not. status_ok(status)) error stop "AMSGrad hypergradient parameter setup failed"
    call objective%initialize(model, train_x, train_target, validation_x, &
        validation_target, options, status)
    if (.not. status_ok(status)) error stop "AMSGrad hypergradient benchmark setup failed"
    parameters = objective%parameters()
    call objective%value_gradient(parameters, value, gradient, status)
    if (.not. status_ok(status)) error stop "AMSGrad hypergradient product failed"
    direction = [0.31_dp, -0.27_dp, 0.13_dp, -0.22_dp, 0.19_dp]
    call objective%jvp(parameters, direction, value, tangent, status)
    if (.not. status_ok(status)) error stop "AMSGrad hypergradient JVP failed"

    oracle_unit = -1
    call get_environment_variable("FORTML_BENCH_AMSGRAD_HYPERGRADIENT_ORACLE", &
    ...
    do repetition = 1, repetitions
        call objective%value_gradient(parameters, value, gradient, status)
        if (.not. status_ok(status)) error stop "AMSGrad hypergradient timing failed"
    end do
    call system_clock(clock_end)
    elapsed = real(clock_end-clock_start, dp)/real(clock_rate, dp) &
        /real(repetitions, dp)
    write (*, '(a,es24.16)') "amsgrad_hypergradient_value_gradient,", elapsed
...
end program fortml_bench_amsgrad_hypergradient
