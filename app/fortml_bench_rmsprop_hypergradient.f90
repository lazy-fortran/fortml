program fortml_bench_rmsprop_hypergradient
    !! Release workload for the fixed full-batch RMSprop hypergradient.
    use, intrinsic :: iso_fortran_env, only: dp => real64, int64
    use fortml_mlp, only: mlp_t
    use fortml_mlp_hypergradient, only: &
        mlp_rmsprop_hypergradient_objective_t, &
        mlp_rmsprop_hypergradient_options_t
    ...
    if (.not. status_ok(status)) error stop "RMSprop hypergradient benchmark initialization failed"
    call model%set_parameters([0.15_dp, -0.1_dp], status)
    if (.not. status_ok(status)) error stop "RMSprop hypergradient parameter setup failed"
    call objective%initialize(model, train_x, train_target, validation_x, &
        validation_target, options, status)
    if (.not. status_ok(status)) error stop "RMSprop hypergradient benchmark setup failed"
    parameters = objective%parameters()
    call objective%value_gradient(parameters, value, gradient, status)
    if (.not. status_ok(status)) error stop "RMSprop hypergradient product failed"
    ...
    do repetition = 1, repetitions
        call objective%value_gradient(parameters, value, gradient, status)
        if (.not. status_ok(status)) error stop "RMSprop hypergradient timing failed"
    end do
...
end program fortml_bench_rmsprop_hypergradient
