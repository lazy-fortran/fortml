program fortml_bench_adaboost_samme
    !! Release workload for the multiclass SAMME weighted-CART contract.
    ...
    do row = 1, repetitions
        ...
        if (.not. status_ok(status)) error stop "SAMME benchmark fit failed"
    end do
    ...
    do row = 1, repetitions*16
        ...
        if (.not. status_ok(status)) error stop "SAMME benchmark prediction failed"
    end do
    call system_clock(clock_end)
    ...
    if (.not. status_ok(status)) error stop "SAMME benchmark labels failed"
    ...
    write (*, '(a,es24.16)') "samme_fit,", fit_seconds
    write (*, '(a,es24.16)') "samme_predict,", predict_seconds
    write (*, '(a,*(1x,es24.16))') "samme_probability_values", probabilities
    write (*, '(a,*(1x,i0))') "samme_prediction_values", predicted
    write (*, '(a,i0)') "samme_stage_count,", model%estimator_count()
    write (*, '(a,es24.16)') "samme_stage_weight,", stage_weight(1)
    write (*, '(a,i0)') "samme_cuda_code,", cuda_code
end program fortml_bench_adaboost_samme
