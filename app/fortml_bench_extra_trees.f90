program fortml_bench_extra_trees
    !! Correctness-gated Extra-Trees classification workload.
    ...
    type(extra_trees_classifier_t) :: model
    ...
    call model%fit(x, labels, status, n_trees=32, max_depth=6, max_features=2, &
        random_splits=64, seed=1729)
    call system_clock(finished)
    if (.not. status_ok(status)) error stop "extra trees benchmark fit failed"
    ...
    if (.not. status_ok(status)) error stop "extra trees benchmark prediction failed"
    call system_clock(started, rate)
    ...
    write (*, '(a,es24.16)') "extra_trees_fit_seconds,", fit_seconds
    write (*, '(a,es24.16)') "extra_trees_predict_seconds,", predict_seconds
    write (*, '(a,es24.16)') "extra_trees_probability_sum_error,", &
        maxval(abs(sum(probabilities, dim=2) - 1.0_dp))
    write (*, '(a,i0)') "extra_trees_query_correct,", &
    ...
    if (status%code /= FORTNUM_NOT_IMPLEMENTED) then
        error stop "extra trees CUDA contract changed unexpectedly"
    end if
    write (*, '(a)') "extra_trees_cuda,unavailable"
end program fortml_bench_extra_trees
