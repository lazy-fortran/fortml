program fortml_bench_composite_operator
    use, intrinsic :: iso_fortran_env, only: dp => real64, int64
    use fortml_kernel_operator, only: kernel_operator_t
    use fortml_kernels, only: kernel_add, kernel_t, &
        make_constant_kernel, make_rbf_kernel
    ...
    real(dp), parameter :: constant_variance = 0.2_dp
    ...
    type(kernel_t) :: rbf_kernel, constant_kernel, composite_kernel
    ...
    allocate(sample_points(n_samples, n_features), input(n_samples), &
        output(n_samples), expected(n_samples))
    ...
    rbf_kernel = make_rbf_kernel( &
        n_features, variance, lengthscale, status)
    if (status%code /= FORTNUM_OK) error stop "RBF kernel construction failed"
    constant_kernel = make_constant_kernel( &
        n_features, constant_variance, status)
    if (status%code /= FORTNUM_OK) then
        error stop "constant kernel construction failed"
    end if
    composite_kernel = kernel_add(rbf_kernel, constant_kernel, status)
    if (status%code /= FORTNUM_OK) error stop "kernel sum construction failed"
    call operator%initialize( &
        sample_points, composite_kernel, diagonal_shift, status, tile_size)
    if (status%code /= FORTNUM_OK) then
        error stop "composite kernel operator initialization failed"
    end if
    ...
    if (maxval(abs(output - expected)) > 3.0e-12_dp*scale) then
        error stop "composite benchmark correctness oracle failed"
    end if
    ...
    if (trim(mode) == "resident") then
        ...
        if (status%code /= FORTNUM_OK) then
            error stop "composite operator data entry failed"
        end if
        ...
        if (status%code /= FORTNUM_OK) then
            error stop "composite operator data exit failed"
        end if
    else
        do repetition = 1, repetitions
            call operator%matvec(input, output)
    ...
    if (sink /= sink) error stop "composite benchmark produced NaN"
    write (*, '(a,i0,a,i0,a,a,a,i0,a,es24.16)') &
        "composite_operator,", n_samples, ",", n_features, ",", &
...
contains
    ...
    subroutine direct_value(row, value)
        ...
        do j = 1, n_samples
            ...
            value = value + (constant_variance + variance*exp( &
...
end program fortml_bench_composite_operator
