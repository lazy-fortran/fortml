program fortml_bench_derivative_gp_rational_quadratic_hvp
    !! CPU timing lane for rational-quadratic mixed-observation GP HVPs.
    ...
    use fortml_kernels, only: kernel_t, make_rational_quadratic_kernel
    use fortnum_status, only: fortnum_status_t, status_ok
    implicit none

    integer, parameter :: n = 4, d = 1, repetitions = 32
    real(dp) :: x_train(n, d), y_train(n, 1), direction(4), hvp(4), seconds
    ...
    x_train(:, 1) = [-0.25_dp, 0.16_dp, 0.73_dp, 1.19_dp]
    y_train(:, 1) = [0.70_dp, -0.20_dp, 0.55_dp, -0.35_dp]
    components = [0, 1, 0, 1]
    direction = [0.13_dp, -0.09_dp, 0.08_dp, -0.05_dp]
    x_query(1, 1) = 0.31_dp
    query_components = [0]

    kernel = make_rational_quadratic_kernel(d, 1.25_dp, 0.81_dp, 1.45_dp, status)
    ...
    call model%fit(x_train, components, y_train, kernel, 0.055_dp, status, &
        jitter=1.0e-10_dp)
    if (.not. status_ok(status)) error stop "rational-quadratic derivative GP fit failed"
    call model%hyperparameter_hvp(direction, hvp, status)
    if (.not. status_ok(status)) error stop "rational-quadratic derivative GP HVP failed"
    ...
    write (*, '(a,i0)') "derivative_gp_rational_quadratic,hvp,cuda_refused,", cuda_code
end program fortml_bench_derivative_gp_rational_quadratic_hvp
