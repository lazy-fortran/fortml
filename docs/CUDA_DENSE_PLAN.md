...
the C ABI. An ordinary GNU build links a stub and returns
`FORTNUM_NOT_IMPLEMENTED` without changing output sentinels. The
`fortml_device` capability probe includes this native availability symbol.
...
two batches on a resident plan, finite-input validation, and complete output
arrays to a `3e-13` absolute tolerance. Machines without a CUDA toolchain are
reported as skipped, never as CPU evidence. This primitive intentionally does
not expose gradients, JVP/VJP/HVP products, or optimizer state; those remain
on the FortAD/FortSym reference path until a full resident derivative graph is
available.
