...
| Basis and pipelines | Polynomial, interaction-polynomial, Chebyshev, Fourier, fixed-state random Fourier, radial, spline, callback, scaling, imputation, one-hot, transactional dense input schemas with unique feature names, horizontal/sequential/column pipelines, bounded named fan-out/fan-in and main-plus-residual-sum DAG branches, basis linear regression, joint basis-pipeline value/gradient/JVP/VJP/HVP products, versioned transactional persistence for preconfigured fitted pipelines, and typed CPU/CUDA dispatch | Conditional branches, sparse views, leakage-safe fit/transform, estimator-wide metadata routing, cloning, structural graph serialization, cross-validation, and resident transforms |
