...
| Trees and ensembles | CART, random forest, Extra-Trees, seeded CART bagging, binary AdaBoost over weighted CART, stump boosting, exact and histogram XGBoost-style objectives, LightGBM-style leaf-wise growth, ranking, monotonic constraints, interaction groups, bounded ordered-gradient integer categorical partitions, sampling, serialization, slicing, warm starts, and validation stopping | OOB/permutation/SHAP workflows, multiclass AdaBoost/SAMME, categorical policies beyond ordered partitions, Tweedie, DART, GOSS, EFB, distributed workers, warm-start LightGBM, and resident GPU histograms |
