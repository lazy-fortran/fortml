...
| Initialization | Checked affine `initialize_linear` state, explicit `initialize_from_pca` centered/whitened linear MLP optimum, a PCA-tied linear autoencoder, finite-feature GP/NTK last-layer and ordinary/Hamiltonian/PINN structure-aware kernel-ridge initialization | Polynomial, spline, Fourier, radial, full GP-posterior, NNGP, physics-consistent, and symplectic initialization with convergence and invariant benchmarks |
