...
| Classification | Binary, softmax, OVR, OVO, multilabel, classifier chains, ordinal heads, five Naive Bayes variants, LDA/QDA, CART, random forest, Extra-Trees, linear/RBF SVM, calibrated heads, and weighted Laplace or variational GP classifiers | Sparse and multioutput labels, kernel SVM/SVR parity, calibrated cross-validation, coupled GP likelihoods, cloning, partial-fit, class-weight parity, and resident GPU training |
...
| Trees and ensembles | CART, random forest, Extra-Trees, seeded CART bagging, binary AdaBoost over weighted CART, stump boosting, exact and histogram XGBoost-style objectives, LightGBM-style leaf-wise growth, ranking, monotonic constraints, interaction groups, sampling, serialization, slicing, warm starts, and validation stopping | OOB/permutation/SHAP workflows, multiclass AdaBoost/SAMME, categorical partitions, Tweedie, DART, GOSS, EFB, distributed workers, warm-start LightGBM, and resident GPU histograms |
