...
| Basis and pipelines | Polynomial, interaction-polynomial, Chebyshev, Fourier, fixed-state random Fourier, radial, spline, callback, scaling, imputation, one-hot, horizontal/sequential/column pipelines, bounded named fan-out/fan-in DAG branches, basis linear regression, joint basis-pipeline value/gradient/JVP/VJP/HVP products, and typed CPU/CUDA dispatch | Residual/conditional branches, sparse views, leakage-safe fit/transform, feature metadata routing, cloning, graph serialization, cross-validation, and resident transforms |
