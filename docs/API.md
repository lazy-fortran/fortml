...
| `mlp_last_layer_gp_initializer_t` | Finite-feature GP/NTK last-layer `fit`, `fit_apply`, `predict`, and posterior predictive variance | Fixed-feature regularization JVP plus exact posterior-variance JVP; named hyperparameter metadata | No VJP (the exposed products are scalar regularization JVPs) | Resident CUDA fit/predict/apply/JVP/variance are typed `FORTNUM_NOT_IMPLEMENTED` refusals |
...
| `gp_derivative_regression_t` | Mean, variance, and LML | Prediction and LML parameter JVP | Prediction parameter VJP and analytic LML hyperparameter gradient | Analytic mixed-observation HVPs for RBF, Matérn 3/2/5/2, periodic, rational-quadratic, linear, constant, polynomial, and supported composites; typed refusals for leaves without second input/parameter products |
...
state. It reuses the MLP HVP path and does not finite-difference.
...
vector-field, JVP, VJP, and HVP-backed `vector_field_jvp`/`vector_field_vjp`
products then act on
...
for RBF, periodic, rational-quadratic, linear, constant, polynomial, spectral-mixture, and sums/products
...
Periodic, rational-quadratic, and spectral-mixture value/first-derivative parameter gradients, query products,
...
kernel coordinates. The rational-quadratic path carries exact radial
`F_s`/`F_ss` products and their three-coordinate parameter-direction products;
`test_derivative_gp_products` checks its independent dense likelihood oracle.
Other leaves return
