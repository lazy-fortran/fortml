...
| `second_derivative_gp_t` | Exact scalar 1-D RBF GP over mixed value/first/second-derivative rows; latent joint covariance | Query-coordinate JVP | Query-coordinate VJP for mean and latent variance | Non-RBF, non-scalar, order >2, and CUDA covariance/solve requests are typed refusals |
