...
out=${OUT:-$repo_dir/.provenance/benchmarks/fortml_composite_operator_nvfortran.csv}
...
mkdir -p "$(dirname "$out")"
"$fc" $flags -module "$build_dir" -o "$build_dir/fortml_bench_composite_operator" \
    "${sources[@]}" "${link_inputs[@]}"

transfer=$("$build_dir/fortml_bench_composite_operator" transfer \
    "${N_SAMPLES:-2048}" "${N_FEATURES:-8}" "${REPETITIONS:-12}")
resident=$("$build_dir/fortml_bench_composite_operator" resident \
    "${N_SAMPLES:-2048}" "${N_FEATURES:-8}" "${REPETITIONS:-12}")
