...
module fortml_adaboost_classifier
    ...
    integer, parameter, public :: ADABOOST_ALGORITHM_SAMME = 1
    integer, parameter, public :: ADABOOST_ALGORITHM_SAMME_R = 2

    !> A discrete AdaBoost classifier with binary/multiclass SAMME and
    !> multiclass probability-update SAMME.R modes.
    !>
    ...
    type, public :: adaboost_classifier_t
        ...
        integer :: algorithm_code = ADABOOST_ALGORITHM_SAMME
    ...
    contains
        ...
        procedure, public :: algorithm => adaboost_algorithm
...
contains
    ...
    subroutine adaboost_fit(self, x, labels, status, n_estimators, max_depth, &
        ...
        requested_algorithm = ADABOOST_ALGORITHM_SAMME
        if (present(algorithm)) requested_algorithm = algorithm
        n = size(x, 1)
        if (n < 2) return
        ...
        if (requested_algorithm /= ADABOOST_ALGORITHM_SAMME .and. &
            requested_algorithm /= ADABOOST_ALGORITHM_SAMME_R) return
        if (any(.not. ieee_is_finite(x))) return
        call sorted_unique_labels(labels, classes)
        n_classes = size(classes)
        if (n_classes < 2) return
        if (requested_algorithm == ADABOOST_ALGORITHM_SAMME_R .and. n_classes < 3) then
            call status_set(status, FORTNUM_DOMAIN_ERROR, &
                "AdaBoost fit: SAMME.R requires at least three classes")
            return
        end if
        ...
        if (requested_algorithm == ADABOOST_ALGORITHM_SAMME_R) then
            allocate(base_probabilities(n, n_classes), log_probabilities(n, n_classes))
        end if
        weights = 1.0_dp
        ...
        do j = 1, requested_estimators
            ...
            if (requested_algorithm == ADABOOST_ALGORITHM_SAMME_R) then
                candidate_alpha(j) = 1.0_dp
            else
                candidate_alpha(j) = learner_alpha
            end if
            fitted_count = j
            if (requested_algorithm == ADABOOST_ALGORITHM_SAMME_R) then
                call candidate_trees(j)%predict_proba(x, base_probabilities, status)
                if (status%code /= FORTNUM_OK) return
                do i = 1, n
                    row_mean = 0.0_dp
                    do k = 1, n_classes
                        log_probabilities(i, k) = log(max(base_probabilities(i, k), epsilon))
                        row_mean = row_mean + log_probabilities(i, k)
                    end do
                    row_mean = row_mean/real(n_classes, dp)
                    true_class = 1
                    do k = 1, n_classes
                        if (labels(i) == classes(k)) then
                            true_class = k
                            exit
                        end if
                    end do
                    update = row_mean - log_probabilities(i, true_class)
                    weights(i) = weights(i)*exp(update)
                end do
            else if (n_classes == 2) then
        ...
        self%algorithm_code = requested_algorithm
    ...
    subroutine adaboost_decision_function_multiclass(self, x, margins, status)
        ...
        type(fortnum_status_t), intent(out) :: status
        integer, allocatable :: labels(:)
        real(dp), allocatable :: probabilities(:, :), log_probabilities(:)
        real(dp) :: row_mean
        integer :: i, j, k
        ...
        margins = 0.0_dp
        do j = 1, self%n_estimators
            if (self%algorithm_code == ADABOOST_ALGORITHM_SAMME_R) then
                allocate(probabilities(size(x, 1), self%n_classes), &
                    log_probabilities(self%n_classes))
                call self%trees(j)%predict_proba(x, probabilities, status)
                if (status%code /= FORTNUM_OK) return
                do i = 1, size(x, 1)
                    row_mean = 0.0_dp
                    do k = 1, self%n_classes
                        log_probabilities(k) = log(max(probabilities(i, k), epsilon_dp()))
                        row_mean = row_mean + log_probabilities(k)
                    end do
                    row_mean = row_mean/real(self%n_classes, dp)
                    do k = 1, self%n_classes
                        margins(i, k) = margins(i, k) + self%alpha(j)* &
                            real(self%n_classes-1, dp)*(log_probabilities(k)-row_mean)
                    end do
                end do
                deallocate(probabilities, log_probabilities)
            else
                call self%trees(j)%predict(x, labels, status)
                if (status%code /= FORTNUM_OK) return
                do i = 1, size(x, 1)
    ...
    subroutine adaboost_predict_proba(self, x, probabilities, status)
        ...
        if (self%n_classes == 2) then
            ...
            block
                ...
                do i = 1, size(x, 1)
                    if (self%algorithm_code == ADABOOST_ALGORITHM_SAMME_R) then
                        shifted = exp(margins(i, :)/real(self%n_classes-1, dp) - &
                            maxval(margins(i, :)/real(self%n_classes-1, dp)))
                    else
    ...
    integer function adaboost_algorithm(self) result(algorithm)
        class(adaboost_classifier_t), intent(in) :: self
        algorithm = self%algorithm_code
    end function adaboost_algorithm
