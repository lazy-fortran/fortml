module fortml_multinomial_naive_bayes
    ...
    contains
        ...
        procedure, public :: partial_fit => multinomial_nb_partial_fit
        procedure, public :: warm_start => multinomial_nb_partial_fit
        procedure, public :: partial_fit_device => multinomial_nb_partial_fit_device
        ...
        procedure, public :: partial_fit_initialized => multinomial_nb_partial_initialized
        procedure, public :: sample_count => multinomial_nb_sample_count
        procedure, public :: batch_count => multinomial_nb_batch_count
        procedure, public :: device_supported => multinomial_nb_device_supported
    end type multinomial_naive_bayes_t

    public :: multinomial_nb_fit
    public :: multinomial_nb_partial_fit
    public :: multinomial_nb_partial_fit_device
...
contains
    ...
    subroutine multinomial_nb_partial_fit(self, x, labels, status, classes, alpha, &
            sample_weight)
        !! Transactionally append a validated nonnegative-count batch.
        ...
        type(multinomial_naive_bayes_t) :: candidate
        ...
        if (n_samples < 1 .or. n_features < 1 .or. size(labels) /= n_samples) then
            call status_set(status, FORTNUM_DOMAIN_ERROR, &
                "MultinomialNB partial_fit: input dimensions are invalid")
            return
        end if
        if (any(.not. ieee_is_finite(x)) .or. any(x < 0.0_dp)) then
            call status_set(status, FORTNUM_DOMAIN_ERROR, &
                "MultinomialNB partial_fit: features must be finite and nonnegative")
            return
        end if
        ...
        if (present(sample_weight)) then
            if (size(sample_weight) /= n_samples .or. &
                ...
                call status_set(status, FORTNUM_DOMAIN_ERROR, &
                    "MultinomialNB partial_fit: sample weights are invalid")
                return
            end if
        ...
        if (.not. ieee_is_finite(sum(batch_weight)) .or. sum(batch_weight) <= 0.0_dp) then
            call status_set(status, FORTNUM_DOMAIN_ERROR, &
                "MultinomialNB partial_fit: batch weight mass must be positive")
            return
        end if
        requested_alpha = DEFAULT_ALPHA
        if (present(alpha)) requested_alpha = alpha
        ...
        if (first_call) then
            if (self%is_fitted .or. .not. present(classes)) then
                call status_set(status, FORTNUM_DOMAIN_ERROR, &
                    "MultinomialNB partial_fit: fresh model and classes are required")
                return
            end if
            if (size(classes) < 2) then
                call status_set(status, FORTNUM_DOMAIN_ERROR, &
                    "MultinomialNB partial_fit: at least two classes are required")
                return
            end if
            do i = 2, size(classes)
                if (classes(i) <= classes(i - 1)) then
                    call status_set(status, FORTNUM_DOMAIN_ERROR, &
                        "MultinomialNB partial_fit: classes must be strictly increasing")
                    return
                end if
            end do
            if (.not. ieee_is_finite(requested_alpha) .or. requested_alpha <= 0.0_dp) then
                call status_set(status, FORTNUM_DOMAIN_ERROR, &
                    "MultinomialNB partial_fit: alpha must be finite and positive")
                return
            end if
            ...
            if (n_features /= self%n_features) then
                call status_set(status, FORTNUM_DOMAIN_ERROR, &
                    "MultinomialNB partial_fit: feature count changed")
                return
            end if
            if (present(classes)) then
                if (size(classes) /= size(self%partial_classes) .or. &
                    ...
                    call status_set(status, FORTNUM_DOMAIN_ERROR, &
                        "MultinomialNB partial_fit: class vocabulary changed")
                    return
                end if
            end if
            ...
            if (present(alpha)) then
                if (.not. ieee_is_finite(alpha) .or. alpha /= self%partial_alpha) then
                    call status_set(status, FORTNUM_DOMAIN_ERROR, &
                        "MultinomialNB partial_fit: alpha cannot change")
                    return
                end if
            end if
        ...
        do i = 1, n_samples
            if (.not. any(labels(i) == candidate_classes)) then
                call status_set(status, FORTNUM_DOMAIN_ERROR, &
                    "MultinomialNB partial_fit: batch contains an unseen class")
                return
            end if
        end do
        n_old = 0
        ...
        call status_set(status, FORTNUM_OK, "")
    end subroutine multinomial_nb_partial_fit

    subroutine multinomial_nb_partial_fit_device(self, device, x, labels, status, &
        ...
        if (.not. device%selected .or. .not. device%available) then
            call status_set(status, FORTNUM_DOMAIN_ERROR, &
                "MultinomialNB partial_fit device: device is not selected")
            return
        end if
        ...
        case (FORTML_DEVICE_CUDA)
            call status_set(status, FORTNUM_NOT_IMPLEMENTED, &
                "MultinomialNB partial_fit device: resident CUDA sufficient statistics are not implemented")
        case default
            call status_set(status, FORTNUM_DOMAIN_ERROR, &
                "MultinomialNB partial_fit device: device kind is invalid")
        end select
    end subroutine multinomial_nb_partial_fit_device
    ...
    logical function multinomial_nb_partial_initialized(self) result(value)
        class(multinomial_naive_bayes_t), intent(in) :: self
        value = self%partial_initialized
    end function multinomial_nb_partial_initialized

    integer function multinomial_nb_sample_count(self) result(value)
        class(multinomial_naive_bayes_t), intent(in) :: self
        value = self%partial_sample_count
    end function multinomial_nb_sample_count

    integer function multinomial_nb_batch_count(self) result(value)
        class(multinomial_naive_bayes_t), intent(in) :: self
        value = self%partial_batch_count
    end function multinomial_nb_batch_count

    logical function multinomial_nb_device_supported(self, device_kind) result(value)
        class(multinomial_naive_bayes_t), intent(in) :: self
        integer, intent(in) :: device_kind
        value = device_kind == FORTML_DEVICE_CPU
    end function multinomial_nb_device_supported
