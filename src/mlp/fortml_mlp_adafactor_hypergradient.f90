...
contains
    ...
    subroutine mlp_adafactor_hypergradient_initialize(self, model, train_x, train_target, &
        ...
        type(mlp_adafactor_hypergradient_metadata_t) :: mlp_adafactor_hypergradient_metadata_t_default

        self%initialized = .false.
        self%layout = mlp_adafactor_hypergradient_metadata_t_default
    ...
    subroutine mlp_optimize_adafactor_hyperparameters(model, train_x, train_target, &
        ...
        !! structure constructors: nvfortran rejects `T()` outright,
        !! and a declared local carries the same default init.
        type(mlp_adafactor_hypergradient_result_t) :: mlp_adafactor_hypergradient_result_t_default

        result = mlp_adafactor_hypergradient_result_t_default
        if (.not. valid_options(options)) then
            if (options%optimizer /= MLP_OPTIMIZER_ADAFACTOR .or. &
                    options%device_kind /= FORTML_DEVICE_CPU) then
                call status_set(status, FORTNUM_NOT_IMPLEMENTED, &
