module fortml_mlp_radam_schedule_hypergradient
    !! Exact products through a fixed full-batch scheduled RAdam trajectory.
    !!
    !! The packed outer coordinates are
    !! `[log(base_rate), log(l2), logit(beta1), logit(beta2), log(epsilon),
    !!   logit(min_rate_fraction), logit(decay_factor)]`.  The objective differentiates the complete
    ...
    integer, parameter, public :: MLP_RADAM_SCHEDULE_HYPERPARAMETER_COUNT = 7
    integer, parameter, public :: MLP_RADAM_SCHEDULE_LOG_BASE_RATE = 1
    integer, parameter, public :: MLP_RADAM_SCHEDULE_LOG_L2 = 2
    integer, parameter, public :: MLP_RADAM_SCHEDULE_LOGIT_BETA1 = 3
    integer, parameter, public :: MLP_RADAM_SCHEDULE_LOGIT_BETA2 = 4
    integer, parameter, public :: MLP_RADAM_SCHEDULE_LOG_EPSILON = 5
    integer, parameter, public :: MLP_RADAM_SCHEDULE_LOGIT_MIN_FRACTION = 6
    integer, parameter, public :: MLP_RADAM_SCHEDULE_LOGIT_DECAY_FACTOR = 7

    type, public :: mlp_radam_schedule_hypergradient_metadata_t
        integer :: parameter_count = MLP_RADAM_SCHEDULE_HYPERPARAMETER_COUNT
        integer :: log_base_rate_index = MLP_RADAM_SCHEDULE_LOG_BASE_RATE
        integer :: log_l2_index = MLP_RADAM_SCHEDULE_LOG_L2
        integer :: logit_beta1_index = MLP_RADAM_SCHEDULE_LOGIT_BETA1
        integer :: logit_beta2_index = MLP_RADAM_SCHEDULE_LOGIT_BETA2
        integer :: log_epsilon_index = MLP_RADAM_SCHEDULE_LOG_EPSILON
        integer :: logit_min_fraction_index = MLP_RADAM_SCHEDULE_LOGIT_MIN_FRACTION
        integer :: logit_decay_factor_index = MLP_RADAM_SCHEDULE_LOGIT_DECAY_FACTOR
    ...
    end type mlp_radam_schedule_hypergradient_metadata_t

    type, public :: mlp_radam_schedule_hypergradient_options_t
        !! Fixed full-batch scheduled RAdam trajectory configuration.
        integer :: steps = 8
    ...
    end type mlp_radam_schedule_hypergradient_options_t

    type, public :: mlp_radam_schedule_hypergradient_result_t
    ...
    end type mlp_radam_schedule_hypergradient_result_t

    type, public :: mlp_radam_schedule_hypergradient_objective_t
        !! FortOpt-compatible validation objective through scheduled RAdam state.
        private
        ...
        type(mlp_radam_schedule_hypergradient_metadata_t) :: layout
        type(mlp_learning_rate_schedule_t) :: schedule
    ...
    contains
        procedure, public :: initialize => mlp_radam_schedule_hypergradient_initialize
        procedure, public :: parameter_count => mlp_radam_schedule_hypergradient_parameter_count
        procedure, public :: metadata => mlp_radam_schedule_hypergradient_metadata
        procedure, public :: parameters => mlp_radam_schedule_hypergradient_parameters
        procedure, public :: value_gradient => mlp_radam_schedule_hypergradient_value_gradient
        procedure, public :: jvp => mlp_radam_schedule_hypergradient_jvp
        procedure, public :: vjp => mlp_radam_schedule_hypergradient_vjp
        procedure, public :: hvp => mlp_radam_schedule_hypergradient_hvp
        procedure, public :: fortopt => mlp_radam_schedule_hypergradient_fortopt
        procedure, public :: is_initialized => mlp_radam_schedule_hypergradient_is_initialized
    end type mlp_radam_schedule_hypergradient_objective_t

    public :: mlp_optimize_radam_schedule_hyperparameters

contains

    subroutine mlp_radam_schedule_hypergradient_initialize(self, model, train_x, train_target, &
            validation_x, validation_target, options, status)
        class(mlp_radam_schedule_hypergradient_objective_t), intent(out) :: self
        ...
        type(mlp_radam_schedule_hypergradient_options_t), intent(in) :: options
        type(fortnum_status_t), intent(out) :: status
        !! Default-initialized instances, standing in for empty
        !! structure constructors: nvfortran rejects `T()` outright,
        !! and a declared local carries the same default init.
        type(mlp_radam_schedule_hypergradient_metadata_t) :: mlp_radam_schedule_hypergradient_metadata_t_default

        self%initialized = .false.
        self%layout = mlp_radam_schedule_hypergradient_metadata_t_default
        if (.not. valid_options(options)) then
            if (options%optimizer /= MLP_OPTIMIZER_RADAM .or. &
                ...
                call status_set(status, FORTNUM_NOT_IMPLEMENTED, &
                    "MLP scheduled RAdam hypergradient: optimizer or device is unsupported")
            else
                call status_set(status, FORTNUM_DOMAIN_ERROR, &
                    "MLP scheduled RAdam hypergradient: options are invalid")
            end if
            return
        end if
        if (.not. valid_data(model, train_x, train_target)) then
            call status_set(status, FORTNUM_DOMAIN_ERROR, &
                "MLP scheduled RAdam hypergradient: training model or data is invalid")
            return
        end if
        if (.not. valid_data(model, validation_x, validation_target)) then
            call status_set(status, FORTNUM_DOMAIN_ERROR, &
                "MLP scheduled RAdam hypergradient: validation model or data is invalid")
            return
        end if
    ...
    end subroutine mlp_radam_schedule_hypergradient_initialize

    integer function mlp_radam_schedule_hypergradient_parameter_count(self) result(count)
        class(mlp_radam_schedule_hypergradient_objective_t), intent(in) :: self

        count = 0
        if (self%initialized) count = self%layout%parameter_count
    end function mlp_radam_schedule_hypergradient_parameter_count

    function mlp_radam_schedule_hypergradient_metadata(self) result(layout)
        class(mlp_radam_schedule_hypergradient_objective_t), intent(in) :: self
        type(mlp_radam_schedule_hypergradient_metadata_t) :: layout

        layout = self%layout
    end function mlp_radam_schedule_hypergradient_metadata

    function mlp_radam_schedule_hypergradient_parameters(self) result(parameters)
        class(mlp_radam_schedule_hypergradient_objective_t), intent(in) :: self
        real(dp), allocatable :: parameters(:)

        allocate(parameters(MLP_RADAM_SCHEDULE_HYPERPARAMETER_COUNT))
    ...
    end function mlp_radam_schedule_hypergradient_parameters

    logical function mlp_radam_schedule_hypergradient_is_initialized(self) result(yes)
        class(mlp_radam_schedule_hypergradient_objective_t), intent(in) :: self
    ...
    end function mlp_radam_schedule_hypergradient_is_initialized

    subroutine mlp_radam_schedule_hypergradient_value_gradient(self, parameters, value, &
            gradient, status)
        class(mlp_radam_schedule_hypergradient_objective_t), intent(inout) :: self
        real(dp), intent(in) :: parameters(:)
        real(dp), intent(out) :: value, gradient(:)
        type(fortnum_status_t), intent(out) :: status
        real(dp) :: direction(MLP_RADAM_SCHEDULE_HYPERPARAMETER_COUNT), tangent
        ...
        if (.not. self%is_initialized()) then
            call status_set(status, FORTNUM_DOMAIN_ERROR, &
                "MLP scheduled RAdam hypergradient: objective is not initialized")
            return
        end if
        if (size(parameters) /= MLP_RADAM_SCHEDULE_HYPERPARAMETER_COUNT) then
            call status_set(status, FORTNUM_DOMAIN_ERROR, &
                "MLP scheduled RAdam hypergradient: packed parameter shape is invalid")
            return
        end if
        if (size(gradient) /= MLP_RADAM_SCHEDULE_HYPERPARAMETER_COUNT) then
            call status_set(status, FORTNUM_DOMAIN_ERROR, &
                "MLP scheduled RAdam hypergradient: packed gradient shape is invalid")
            return
        end if
        if (any(.not. ieee_is_finite(parameters))) then
            call status_set(status, FORTNUM_DOMAIN_ERROR, &
                "MLP scheduled RAdam hypergradient: packed parameters are not finite")
            return
        end if
        call radam_forward(self, parameters, direction, value, tangent, gradient, status)
    end subroutine mlp_radam_schedule_hypergradient_value_gradient

    subroutine mlp_radam_schedule_hypergradient_jvp(self, parameters, direction, value, &
            tangent, status)
        class(mlp_radam_schedule_hypergradient_objective_t), intent(inout) :: self
        real(dp), intent(in) :: parameters(:), direction(:)
        real(dp), intent(out) :: value, tangent
        type(fortnum_status_t), intent(out) :: status
        real(dp) :: gradient(MLP_RADAM_SCHEDULE_HYPERPARAMETER_COUNT)
        ...
        if (.not. self%is_initialized()) then
            call status_set(status, FORTNUM_DOMAIN_ERROR, &
                "MLP scheduled RAdam hypergradient JVP: objective is not initialized")
            return
        end if
        if (size(parameters) /= MLP_RADAM_SCHEDULE_HYPERPARAMETER_COUNT .or. &
                size(direction) /= MLP_RADAM_SCHEDULE_HYPERPARAMETER_COUNT) then
            call status_set(status, FORTNUM_DOMAIN_ERROR, &
                "MLP scheduled RAdam hypergradient JVP: packed shape is invalid")
            return
        end if
        if (any(.not. ieee_is_finite(parameters)) .or. &
                any(.not. ieee_is_finite(direction))) then
            call status_set(status, FORTNUM_DOMAIN_ERROR, &
                "MLP scheduled RAdam hypergradient JVP: packed values are not finite")
            return
        end if
        call radam_forward(self, parameters, direction, value, tangent, gradient, status)
    end subroutine mlp_radam_schedule_hypergradient_jvp

    subroutine mlp_radam_schedule_hypergradient_vjp(self, parameters, output_bar, gradient, &
            status)
        class(mlp_radam_schedule_hypergradient_objective_t), intent(inout) :: self
        ...
        if (.not. ieee_is_finite(output_bar)) then
            call status_set(status, FORTNUM_DOMAIN_ERROR, &
                "MLP scheduled RAdam hypergradient VJP: cotangent is invalid")
            return
        end if
        ...
        call status_set(status, FORTNUM_OK, "")
    end subroutine mlp_radam_schedule_hypergradient_vjp

    subroutine mlp_radam_schedule_hypergradient_hvp(self, parameters, direction, &
            product, status)
        !! Outer hyper-HVP is refused until third network derivatives exist.
        !! The trajectory JVP is exact; differentiating its MLP HVP with
        !! respect to the trajectory would require a third derivative of the
        !! nonlinear loss.  Never approximate this product by finite differences.
        class(mlp_radam_schedule_hypergradient_objective_t), intent(in) :: self
        real(dp), intent(in) :: parameters(:), direction(:)
        ...
        product = 0.0_dp
        if (size(parameters) /= MLP_RADAM_SCHEDULE_HYPERPARAMETER_COUNT .or. &
                size(direction) /= MLP_RADAM_SCHEDULE_HYPERPARAMETER_COUNT .or. &
                size(product) /= MLP_RADAM_SCHEDULE_HYPERPARAMETER_COUNT .or. &
                any(.not. ieee_is_finite(parameters)) .or. &
                any(.not. ieee_is_finite(direction))) then
            call status_set(status, FORTNUM_DOMAIN_ERROR, &
                "MLP scheduled RAdam hypergradient HVP: packed shape is invalid")
            return
        end if
        if (.not. self%is_initialized()) then
            call status_set(status, FORTNUM_DOMAIN_ERROR, &
                "MLP scheduled RAdam hypergradient HVP: objective is not initialized")
            return
        end if
        call status_set(status, FORTNUM_NOT_IMPLEMENTED, &
            "MLP scheduled RAdam hypergradient HVP requires third network derivatives")
    end subroutine mlp_radam_schedule_hypergradient_hvp

    subroutine mlp_radam_schedule_hypergradient_fortopt(self, objective, status)
        class(mlp_radam_schedule_hypergradient_objective_t), target, intent(inout) :: self
        ...
        call objective%initialize_context(MLP_RADAM_SCHEDULE_HYPERPARAMETER_COUNT, self, &
            mlp_radam_schedule_hypergradient_context_callback, status)
    end subroutine mlp_radam_schedule_hypergradient_fortopt

    subroutine mlp_radam_schedule_hypergradient_context_callback(context, parameters, value, &
            gradient, status)
        ...
        select type (adapter => context)
            type is (mlp_radam_schedule_hypergradient_objective_t)
            call adapter%value_gradient(parameters, value, gradient, status)
        class default
            call status_set(status, FORTNUM_DOMAIN_ERROR, &
                "MLP scheduled RAdam hypergradient: context has the wrong type")
        end select
    end subroutine mlp_radam_schedule_hypergradient_context_callback

    subroutine mlp_optimize_radam_schedule_hyperparameters(model, train_x, train_target, &
        ...
        real(dp), intent(in) :: validation_x(:, :), validation_target(:, :)
        type(mlp_radam_schedule_hypergradient_options_t), intent(in) :: options
        type(mlp_radam_schedule_hypergradient_result_t), intent(out) :: result
        type(fortnum_status_t), intent(out) :: status
        type(mlp_radam_schedule_hypergradient_objective_t), target :: adapter
        type(objective_t) :: objective
        type(lbfgsb_t) :: optimizer
        type(lbfgsb_options_t) :: optimizer_options
        type(lbfgsb_result_t) :: optimizer_result
        real(dp) :: parameters(MLP_RADAM_SCHEDULE_HYPERPARAMETER_COUNT)
        real(dp) :: lower(MLP_RADAM_SCHEDULE_HYPERPARAMETER_COUNT)
        real(dp) :: upper(MLP_RADAM_SCHEDULE_HYPERPARAMETER_COUNT)
        real(dp) :: gradient(MLP_RADAM_SCHEDULE_HYPERPARAMETER_COUNT)
        !! Default-initialized instances, standing in for empty
        !! structure constructors: nvfortran rejects `T()` outright,
        !! and a declared local carries the same default init.
        type(mlp_radam_schedule_hypergradient_result_t) :: mlp_radam_schedule_hypergradient_result_t_default

        result = mlp_radam_schedule_hypergradient_result_t_default
        if (.not. valid_options(options)) then
            if (options%optimizer /= MLP_OPTIMIZER_RADAM .or. &
                    options%device_kind /= FORTML_DEVICE_CPU) then
                call status_set(status, FORTNUM_NOT_IMPLEMENTED, &
                    "MLP scheduled RAdam hyperparameter optimization: unsupported optimizer/device")
            else
                call status_set(status, FORTNUM_DOMAIN_ERROR, &
                    "MLP scheduled RAdam hyperparameter optimization: options are invalid")
            end if
            return
        end if
        ...
        result%log_base_rate = parameters(MLP_RADAM_SCHEDULE_LOG_BASE_RATE)
        result%log_l2 = parameters(MLP_RADAM_SCHEDULE_LOG_L2)
        result%logit_beta1 = parameters(MLP_RADAM_SCHEDULE_LOGIT_BETA1)
        result%logit_beta2 = parameters(MLP_RADAM_SCHEDULE_LOGIT_BETA2)
        result%log_epsilon = parameters(MLP_RADAM_SCHEDULE_LOG_EPSILON)
        ...
        result%min_rate_fraction = logit_to_probability( &
            parameters(MLP_RADAM_SCHEDULE_LOGIT_MIN_FRACTION))
        result%decay_factor = logit_to_probability( &
            parameters(MLP_RADAM_SCHEDULE_LOGIT_DECAY_FACTOR))
        if (.not. result%converged) then
            call status_set(status, FORTNUM_CONVERGENCE_ERROR, &
                "MLP scheduled RAdam hyperparameter optimization: iteration limit reached")
            return
        end if
        if (.not. ieee_is_finite(result%objective) .or. &
                .not. ieee_is_finite(result%gradient_norm)) then
            call status_set(status, FORTNUM_CONVERGENCE_ERROR, &
                "MLP scheduled RAdam hyperparameter optimization: result is not finite")
            return
        end if
        call status_set(status, FORTNUM_OK, "")
    end subroutine mlp_optimize_radam_schedule_hyperparameters

    subroutine radam_forward(self, parameters, direction, value, tangent, gradient, status)
        !! Forward-mode propagation of all seven packed hyperparameter tangents.
        class(mlp_radam_schedule_hypergradient_objective_t), intent(inout) :: self
        real(dp), intent(in) :: parameters(:), direction(:)
        ...
        real(dp) :: min_fraction, decay_factor, min_fraction_dot, decay_factor_dot
        real(dp) :: schedule_rate, schedule_rate_dot
        ...
        tangent = 0.0_dp
        gradient = 0.0_dp
        if (.not. finite_parameters(parameters, learning_rate, l2, beta1, beta2, epsilon, &
                min_fraction, decay_factor)) then
            call status_set(status, FORTNUM_DOMAIN_ERROR, &
                "MLP scheduled RAdam hypergradient: packed parameters are invalid")
            return
        end if
        ...
        allocate(theta_dot(n_parameters, MLP_RADAM_SCHEDULE_HYPERPARAMETER_COUNT))
        allocate(first(n_parameters), second(n_parameters))
        allocate(first_dot(n_parameters, MLP_RADAM_SCHEDULE_HYPERPARAMETER_COUNT), &
            second_dot(n_parameters, MLP_RADAM_SCHEDULE_HYPERPARAMETER_COUNT))
        ...
        do step = 1, self%layout%inner_steps
            ...
            if (status%code /= FORTNUM_OK) return
            call schedule%rate_with_derivatives(step, learning_rate, schedule_rate, &
                d_base_rate, d_min_fraction, d_decay_factor, status)
            if (status%code /= FORTNUM_OK) return
            ...
            do parameter_index = 1, MLP_RADAM_SCHEDULE_HYPERPARAMETER_COUNT
                l2_dot = 0.0_dp
                beta1_dot = 0.0_dp
                beta2_dot = 0.0_dp
                if (parameter_index == MLP_RADAM_SCHEDULE_LOG_L2) l2_dot = l2
                if (parameter_index == MLP_RADAM_SCHEDULE_LOGIT_BETA1) beta1_dot = beta1*(1.0_dp-beta1)
                if (parameter_index == MLP_RADAM_SCHEDULE_LOGIT_BETA2) beta2_dot = beta2*(1.0_dp-beta2)
            ...
            if (any(second_hat < 0.0_dp)) then
                call status_set(status, FORTNUM_DOMAIN_ERROR, &
                    "MLP scheduled RAdam hypergradient: second moment is negative")
                return
            end if
            sqrt_second = sqrt(second_hat)
            if (any(sqrt_second == 0.0_dp)) then
                call status_set(status, FORTNUM_NOT_IMPLEMENTED, &
                    "MLP scheduled RAdam hypergradient: zero second-moment square-root derivative")
                return
            end if
            ...
            if (abs(rho_t-4.0_dp) <= self%layout%rho_boundary_tolerance) then
                call status_set(status, FORTNUM_NOT_IMPLEMENTED, &
                    "MLP scheduled RAdam hypergradient: rho_t branch boundary is nonsmooth")
                return
            end if
            ...
            if (rectified) then
                ...
                if (rectification_argument <= 0.0_dp .or. &
                    ...
                    call status_set(status, FORTNUM_DOMAIN_ERROR, &
                        "MLP scheduled RAdam hypergradient: invalid rectification factor")
                    return
                end if
            ...
            do parameter_index = 1, MLP_RADAM_SCHEDULE_HYPERPARAMETER_COUNT
                ...
                beta2_dot = 0.0_dp
                epsilon_dot = 0.0_dp
                if (parameter_index == MLP_RADAM_SCHEDULE_LOG_BASE_RATE) &
                    learning_rate_dot = d_base_rate*learning_rate
                if (parameter_index == MLP_RADAM_SCHEDULE_LOG_L2) l2_dot = l2
                if (parameter_index == MLP_RADAM_SCHEDULE_LOGIT_BETA1) beta1_dot = beta1*(1.0_dp-beta1)
                if (parameter_index == MLP_RADAM_SCHEDULE_LOGIT_BETA2) beta2_dot = beta2*(1.0_dp-beta2)
                if (parameter_index == MLP_RADAM_SCHEDULE_LOG_EPSILON) epsilon_dot = epsilon
                min_fraction_dot = 0.0_dp
                decay_factor_dot = 0.0_dp
                if (parameter_index == MLP_RADAM_SCHEDULE_LOGIT_MIN_FRACTION) &
                    min_fraction_dot = min_fraction*(1.0_dp-min_fraction)
                if (parameter_index == MLP_RADAM_SCHEDULE_LOGIT_DECAY_FACTOR) &
                ...
                schedule_rate_dot = learning_rate_dot + &
                    d_min_fraction*min_fraction_dot + d_decay_factor*decay_factor_dot
                theta_dot(:, parameter_index) = theta_dot(:, parameter_index) - &
                    schedule_rate_dot*update-schedule_rate*update_dot
            end do
            theta = theta-schedule_rate*update
            if (any(.not. ieee_is_finite(theta)) .or. &
                ...
                call status_set(status, FORTNUM_DOMAIN_ERROR, &
                    "MLP scheduled RAdam hypergradient: trajectory is not finite")
                return
            end if
        end do
        call self%model%set_parameters(theta, status)
        ...
        if (.not. ieee_is_finite(value) .or. any(.not. ieee_is_finite(gradient)) .or. &
            ...
            call status_set(status, FORTNUM_DOMAIN_ERROR, &
                "MLP scheduled RAdam hypergradient: trajectory product is not finite")
            return
        end if
        call status_set(status, FORTNUM_OK, "")
    end subroutine radam_forward

    logical function finite_parameters(parameters, learning_rate, l2, beta1, beta2, epsilon, &
        ...
        min_fraction = 0.0_dp
        decay_factor = 0.0_dp
        valid = size(parameters) == MLP_RADAM_SCHEDULE_HYPERPARAMETER_COUNT
        if (.not. valid) return
        if (any(.not. ieee_is_finite(parameters))) then
            valid = .false.
            return
        end if
        learning_rate = exp(parameters(MLP_RADAM_SCHEDULE_LOG_BASE_RATE))
        l2 = exp(parameters(MLP_RADAM_SCHEDULE_LOG_L2))
        beta1 = logit_to_probability(parameters(MLP_RADAM_SCHEDULE_LOGIT_BETA1))
        beta2 = logit_to_probability(parameters(MLP_RADAM_SCHEDULE_LOGIT_BETA2))
        epsilon = exp(parameters(MLP_RADAM_SCHEDULE_LOG_EPSILON))
        min_fraction = logit_to_probability(parameters(MLP_RADAM_SCHEDULE_LOGIT_MIN_FRACTION))
        decay_factor = logit_to_probability(parameters(MLP_RADAM_SCHEDULE_LOGIT_DECAY_FACTOR))
        valid = ieee_is_finite(learning_rate) .and. ieee_is_finite(l2) .and. &
            ...
            beta2 > 0.0_dp .and. beta2 < 1.0_dp .and. epsilon > 0.0_dp .and. &
            min_fraction >= 0.0_dp .and. min_fraction <= 1.0_dp .and. &
    ...
    logical function valid_options(options) result(valid)
        ...
        if (.not. options%schedule%valid()) return
        ...
        if (options%base_rate <= 0.0_dp .or. options%l2 < 0.0_dp .or. &
        ...
        if (.not. ieee_is_finite(options%base_rate) .or. &
        ...
        if (.not. valid) return
        valid = bounds_valid(options%lower_logit_min_fraction, options%upper_logit_min_fraction, &
            probability_logit(interior_probability(options%schedule%min_rate_fraction)))
        if (.not. valid) return
        valid = bounds_valid(options%lower_logit_decay_factor, options%upper_logit_decay_factor, &
    ...
    pure real(dp) function interior_probability(value) result(probability)
        real(dp), intent(in) :: value

        ! Keep the packed logit strictly inside the default FortOpt bounds;
        ! schedule families that do not use a field still carry its typed
        ! coordinate for a stable seven-slot layout.
        probability = min(max(value, 1.0e-5_dp), 1.0_dp-1.0e-5_dp)
    end function interior_probability

end module fortml_mlp_radam_schedule_hypergradient
