module fortml_mlp_hypergradient
    ...
    !! value/gradient path uses exact trajectory sensitivities; no finite
    !! differences, optimizer fallback, or hidden host/device transfer is
    !! used.  CUDA and stochastic/minibatch optimizer trajectories remain
    !! explicit refusal paths until their device-aware contracts exist.
    ...
    ! The legacy three-parameter AdamW contract above deliberately keeps
    ! beta values fixed.  The full contract below uses a separate packed
    ! layout so that beta sensitivities cannot be confused with that API.
    integer, parameter, public :: MLP_ADAMW_FULL_HYPERPARAMETER_COUNT = 5
    integer, parameter, public :: MLP_ADAMW_FULL_LOG_LEARNING_RATE = 1
    integer, parameter, public :: MLP_ADAMW_FULL_LOG_L2 = 2
    integer, parameter, public :: MLP_ADAMW_FULL_LOG_WEIGHT_DECAY = 3
    integer, parameter, public :: MLP_ADAMW_FULL_LOGIT_BETA1 = 4
    integer, parameter, public :: MLP_ADAMW_FULL_LOGIT_BETA2 = 5
    ! Short aliases make the differentiable-beta layout easy to discover.
    integer, parameter, public :: MLP_ADAMW_BETA_HYPERPARAMETER_COUNT = &
        MLP_ADAMW_FULL_HYPERPARAMETER_COUNT
    integer, parameter, public :: MLP_ADAMW_LOGIT_BETA1 = MLP_ADAMW_FULL_LOGIT_BETA1
    integer, parameter, public :: MLP_ADAMW_LOGIT_BETA2 = MLP_ADAMW_FULL_LOGIT_BETA2
    ...
    type, public :: mlp_adamw_full_hypergradient_metadata_t
        !! Packed layout with differentiable AdamW moment coefficients.
        !!
        !! The beta values use unconstrained logits, `log(beta/(1-beta))`.
        !! This keeps every L-BFGS-B iterate in the open interval (0,1).
        integer :: parameter_count = MLP_ADAMW_FULL_HYPERPARAMETER_COUNT
        integer :: log_learning_rate_index = MLP_ADAMW_FULL_LOG_LEARNING_RATE
        integer :: log_l2_index = MLP_ADAMW_FULL_LOG_L2
        integer :: log_weight_decay_index = MLP_ADAMW_FULL_LOG_WEIGHT_DECAY
        integer :: logit_beta1_index = MLP_ADAMW_FULL_LOGIT_BETA1
        integer :: logit_beta2_index = MLP_ADAMW_FULL_LOGIT_BETA2
        integer :: inner_steps = 0
    end type mlp_adamw_full_hypergradient_metadata_t

    type, public :: mlp_adamw_full_hypergradient_options_t
        !! Fixed full-batch AdamW trajectory with beta hypergradients.
        integer :: steps = 8
        real(dp) :: learning_rate = 1.0e-2_dp
        real(dp) :: l2 = 1.0e-4_dp
        real(dp) :: weight_decay = 1.0e-2_dp
        ...
        real(dp) :: step_tolerance = 1.0e-12_dp
        real(dp) :: objective_tolerance = 1.0e-12_dp
    end type mlp_adamw_full_hypergradient_options_t

    type, public :: mlp_adamw_full_hypergradient_result_t
        logical :: converged = .false.
        integer :: iterations = 0
        integer :: line_search_evaluations = 0
        real(dp) :: objective = huge(1.0_dp)
    ...
    end type mlp_adamw_full_hypergradient_result_t
    ...
    type, public :: mlp_adamw_full_hypergradient_objective_t
        !! Exact value/gradient products through AdamW including beta1/beta2.
        !!
        !! `base` owns the validated model/data and the fixed initial state;
        !! keeping it as a separate object preserves the legacy three-value
        !! AdamW layout while sharing its data-shape and model contracts.
        private
        type(mlp_adamw_hypergradient_objective_t) :: base
        type(mlp_adamw_full_hypergradient_metadata_t) :: layout
        real(dp) :: initial_log_learning_rate = 0.0_dp
        real(dp) :: initial_log_l2 = 0.0_dp
        real(dp) :: initial_log_weight_decay = 0.0_dp
        ...
        logical :: initialized = .false.
    contains
        procedure, public :: initialize => mlp_adamw_full_hypergradient_initialize
        procedure, public :: parameter_count => mlp_adamw_full_hypergradient_parameter_count
        procedure, public :: metadata => mlp_adamw_full_hypergradient_metadata
        procedure, public :: parameters => mlp_adamw_full_hypergradient_parameters
    ...
    public :: mlp_optimize_adamw_full_hyperparameters
...
contains
    ...
    subroutine mlp_adamw_full_hypergradient_initialize(self, model, train_x, &
            train_target, validation_x, validation_target, options, status)
        class(mlp_adamw_full_hypergradient_objective_t), intent(out) :: self
        type(mlp_t), target, intent(inout) :: model
        real(dp), intent(in) :: train_x(:, :), train_target(:, :)
        real(dp), intent(in) :: validation_x(:, :), validation_target(:, :)
        ...
        if (.not. valid_adamw_full_options(options)) then
            if (options%optimizer /= MLP_OPTIMIZER_ADAMW .or. &
                options%device_kind /= FORTML_DEVICE_CPU) then
                call status_set(status, FORTNUM_NOT_IMPLEMENTED, &
                    "MLP full AdamW hypergradient: optimizer or device is unsupported")
            else
                call status_set(status, FORTNUM_DOMAIN_ERROR, &
                    "MLP full AdamW hypergradient: options are invalid")
            end if
            return
        end if
        base_options%steps = options%steps
        base_options%learning_rate = options%learning_rate
        base_options%l2 = options%l2
        base_options%weight_decay = options%weight_decay
        base_options%beta1 = options%beta1
        base_options%beta2 = options%beta2
        base_options%epsilon = options%epsilon
        base_options%lower_log_learning_rate = options%lower_log_learning_rate
        base_options%upper_log_learning_rate = options%upper_log_learning_rate
        base_options%lower_log_l2 = options%lower_log_l2
        base_options%upper_log_l2 = options%upper_log_l2
        base_options%lower_log_weight_decay = options%lower_log_weight_decay
        base_options%upper_log_weight_decay = options%upper_log_weight_decay
        base_options%optimizer = options%optimizer
        base_options%device_kind = options%device_kind
        call self%base%initialize(model, train_x, train_target, validation_x, &
            validation_target, base_options, status)
        if (status%code /= FORTNUM_OK) return
        self%layout%inner_steps = options%steps
        self%initial_log_learning_rate = log(options%learning_rate)
        self%initial_log_l2 = log(options%l2)
        ...
        self%initialized = .true.
        call status_set(status, FORTNUM_OK, "")
    end subroutine mlp_adamw_full_hypergradient_initialize

    integer function mlp_adamw_full_hypergradient_parameter_count(self) result(count)
        class(mlp_adamw_full_hypergradient_objective_t), intent(in) :: self

        count = 0
        if (self%initialized) count = self%layout%parameter_count
    end function mlp_adamw_full_hypergradient_parameter_count

    function mlp_adamw_full_hypergradient_metadata(self) result(layout)
        class(mlp_adamw_full_hypergradient_objective_t), intent(in) :: self
        type(mlp_adamw_full_hypergradient_metadata_t) :: layout

        layout = self%layout
    end function mlp_adamw_full_hypergradient_metadata

    function mlp_adamw_full_hypergradient_parameters(self) result(parameters)
        class(mlp_adamw_full_hypergradient_objective_t), intent(in) :: self
        real(dp), allocatable :: parameters(:)

        allocate(parameters(MLP_ADAMW_FULL_HYPERPARAMETER_COUNT))
        parameters = [self%initial_log_learning_rate, self%initial_log_l2, &
            self%initial_log_weight_decay, self%initial_logit_beta1, &
            self%initial_logit_beta2]
    end function mlp_adamw_full_hypergradient_parameters

    logical function mlp_adamw_full_hypergradient_is_initialized(self) result(yes)
        class(mlp_adamw_full_hypergradient_objective_t), intent(in) :: self

        yes = self%initialized .and. self%base%is_initialized()
    end function mlp_adamw_full_hypergradient_is_initialized

    subroutine mlp_adamw_full_hypergradient_value_gradient(self, parameters, value, &
            gradient, status)
        ...
        if (.not. self%is_initialized()) then
            call status_set(status, FORTNUM_DOMAIN_ERROR, &
                "MLP full AdamW hypergradient: objective is not initialized")
            return
        end if
        if (size(parameters) /= MLP_ADAMW_FULL_HYPERPARAMETER_COUNT .or. &
            size(gradient) /= MLP_ADAMW_FULL_HYPERPARAMETER_COUNT .or. &
            any(.not. ieee_is_finite(parameters))) then
            call status_set(status, FORTNUM_DOMAIN_ERROR, &
                "MLP full AdamW hypergradient: packed parameter shape is invalid")
            return
        end if
        call adamw_full_reverse_value_gradient(self, parameters, value, gradient, status)
    end subroutine mlp_adamw_full_hypergradient_value_gradient

    subroutine mlp_adamw_full_hypergradient_jvp(self, parameters, direction, value, &
            tangent, status)
        class(mlp_adamw_full_hypergradient_objective_t), intent(inout) :: self
        real(dp), intent(in) :: parameters(:), direction(:)
        real(dp), intent(out) :: value, tangent
        ...
        if (.not. self%is_initialized()) then
            call status_set(status, FORTNUM_DOMAIN_ERROR, &
                "MLP full AdamW hypergradient JVP: objective is not initialized")
            return
        end if
        if (size(parameters) /= MLP_ADAMW_FULL_HYPERPARAMETER_COUNT .or. &
            ...
            any(.not. ieee_is_finite(direction))) then
            call status_set(status, FORTNUM_DOMAIN_ERROR, &
                "MLP full AdamW hypergradient JVP: packed shape is invalid")
            return
        end if
        call adamw_full_forward(self, parameters, direction, value, tangent, gradient, status)
    end subroutine mlp_adamw_full_hypergradient_jvp

    subroutine mlp_adamw_full_hypergradient_vjp(self, parameters, output_bar, gradient, &
            status)
        class(mlp_adamw_full_hypergradient_objective_t), intent(inout) :: self
        real(dp), intent(in) :: parameters(:), output_bar
        real(dp), intent(out) :: gradient(:)
        ...
        if (.not. ieee_is_finite(output_bar)) then
            call status_set(status, FORTNUM_DOMAIN_ERROR, &
                "MLP full AdamW hypergradient VJP: cotangent is invalid")
            return
        end if
        call self%value_gradient(parameters, value, gradient, status)
        if (status%code /= FORTNUM_OK) return
    ...
    subroutine mlp_adamw_full_hypergradient_fortopt(self, objective, status)
        ...
        call objective%initialize_context(MLP_ADAMW_FULL_HYPERPARAMETER_COUNT, self, &
            mlp_adamw_full_hypergradient_context_callback, status)
    end subroutine mlp_adamw_full_hypergradient_fortopt

    subroutine mlp_adamw_full_hypergradient_context_callback(context, parameters, value, &
            gradient, status)
        class(*), intent(inout) :: context
        real(dp), intent(in) :: parameters(:)
        real(dp), intent(out) :: value, gradient(:)
        ...
        select type (adapter => context)
            type is (mlp_adamw_full_hypergradient_objective_t)
            call adapter%value_gradient(parameters, value, gradient, status)
        class default
            call status_set(status, FORTNUM_DOMAIN_ERROR, &
                "MLP full AdamW hypergradient: context has the wrong type")
        end select
    end subroutine mlp_adamw_full_hypergradient_context_callback

    subroutine mlp_optimize_adamw_full_hyperparameters(model, train_x, train_target, &
            validation_x, validation_target, options, result, status)
        type(mlp_t), target, intent(inout) :: model
        real(dp), intent(in) :: train_x(:, :), train_target(:, :)
        real(dp), intent(in) :: validation_x(:, :), validation_target(:, :)
        type(mlp_adamw_full_hypergradient_options_t), intent(in) :: options
        type(mlp_adamw_full_hypergradient_result_t), intent(out) :: result
        type(fortnum_status_t), intent(out) :: status
        type(mlp_adamw_full_hypergradient_objective_t), target :: adapter
        type(objective_t) :: objective
        type(lbfgsb_t) :: optimizer
        type(lbfgsb_options_t) :: optimizer_options
        type(lbfgsb_result_t) :: optimizer_result
        real(dp) :: parameters(MLP_ADAMW_FULL_HYPERPARAMETER_COUNT)
        ...
        result%line_search_evaluations = optimizer_result%line_search_evaluations
        result%gradient_norm = sqrt(sum(gradient*gradient))
        result%log_learning_rate = parameters(MLP_ADAMW_FULL_LOG_LEARNING_RATE)
        result%log_l2 = parameters(MLP_ADAMW_FULL_LOG_L2)
        result%log_weight_decay = parameters(MLP_ADAMW_FULL_LOG_WEIGHT_DECAY)
        result%logit_beta1 = parameters(MLP_ADAMW_FULL_LOGIT_BETA1)
        result%logit_beta2 = parameters(MLP_ADAMW_FULL_LOGIT_BETA2)
        result%learning_rate = exp(result%log_learning_rate)
        result%l2 = exp(result%log_l2)
        ...
        if (.not. result%converged) then
            call status_set(status, FORTNUM_CONVERGENCE_ERROR, &
                "MLP full AdamW hyperparameter optimization: iteration limit reached")
            return
        end if
        call status_set(status, FORTNUM_OK, "")
    end subroutine mlp_optimize_adamw_full_hyperparameters

    subroutine adamw_full_reverse_value_gradient(self, parameters, value, gradient, status)
        !! Assemble the scalar VJP from the exact forward sensitivities.
        !!
        !! There are only five outer variables, so reusing the forward-mode
        !! recurrence keeps this adapter short and, importantly, makes the
        !! value/gradient and JVP paths share one analytically differentiated
        !! AdamW state transition.  No finite differences are used.
        class(mlp_adamw_full_hypergradient_objective_t), intent(inout) :: self
        real(dp), intent(in) :: parameters(:)
        real(dp), intent(out) :: value, gradient(:)
        type(fortnum_status_t), intent(out) :: status
        real(dp) :: tangent
        real(dp) :: zero_direction(MLP_ADAMW_FULL_HYPERPARAMETER_COUNT)

        zero_direction = 0.0_dp
        call adamw_full_forward(self, parameters, zero_direction, value, tangent, gradient, status)
    end subroutine adamw_full_reverse_value_gradient

    subroutine adamw_full_forward(self, parameters, direction, value, tangent, gradient, &
            status)
        !! Forward JVP through full-batch AdamW, including beta logits.
        class(mlp_adamw_full_hypergradient_objective_t), intent(inout) :: self
        real(dp), intent(in) :: parameters(:), direction(:)
        ...
        real(dp), allocatable :: update(:), update_dot(:), first_hat(:), second_hat(:)
        real(dp), allocatable :: first_hat_dot(:), second_hat_dot(:)
        real(dp) :: learning_rate, l2, weight_decay, beta1, beta2
        real(dp) :: learning_rate_dot, l2_dot, weight_decay_dot, beta1_dot, beta2_dot
        real(dp) :: c1, c2, c1_dot, c2_dot, train_value, l2_gradient, scalar_hvp
        real(dp), allocatable :: denominator_dot(:)
        integer :: n_parameters, step, parameter_index

        value = huge(1.0_dp)
        tangent = 0.0_dp
        gradient = 0.0_dp
        if (.not. finite_adamw_full_parameters(parameters, learning_rate, l2, &
            weight_decay, beta1, beta2)) then
            call status_set(status, FORTNUM_DOMAIN_ERROR, &
                "MLP full AdamW hypergradient: packed parameters are invalid")
            return
        end if
        n_parameters = size(self%base%initial_parameters)
        allocate(theta, source=self%base%initial_parameters)
        allocate(theta_dot(n_parameters, MLP_ADAMW_FULL_HYPERPARAMETER_COUNT))
        allocate(first(n_parameters), second(n_parameters), first_previous(n_parameters), &
            second_previous(n_parameters))
        allocate(first_dot(n_parameters, MLP_ADAMW_FULL_HYPERPARAMETER_COUNT), &
            second_dot(n_parameters, MLP_ADAMW_FULL_HYPERPARAMETER_COUNT))
        allocate(raw_gradient(n_parameters), gradient_dot(n_parameters), hvp(n_parameters))
        allocate(sqrt_second(n_parameters), denominator(n_parameters), update(n_parameters), &
            update_dot(n_parameters), first_hat(n_parameters), second_hat(n_parameters), &
            first_hat_dot(n_parameters), second_hat_dot(n_parameters), denominator_dot(n_parameters))
        theta_dot = 0.0_dp
        first = 0.0_dp
        second = 0.0_dp
        first_dot = 0.0_dp
        ...
        do step = 1, self%layout%inner_steps
            ...
            do parameter_index = 1, MLP_ADAMW_FULL_HYPERPARAMETER_COUNT
                l2_dot = 0.0_dp
                if (parameter_index == MLP_ADAMW_FULL_LOG_L2) l2_dot = l2
                call mlp_loss_hvp(self%base%model, self%base%train_x, &
                    self%base%train_target, l2, theta_dot(:, parameter_index), l2_dot, &
            ...
            theta = (1.0_dp-learning_rate*weight_decay)*theta-learning_rate*update
            if (any(.not. ieee_is_finite(theta)) .or. any(.not. ieee_is_finite(theta_dot))) then
                call status_set(status, FORTNUM_DOMAIN_ERROR, &
                    "MLP full AdamW hypergradient: trajectory is not finite")
                return
            end if
        end do
        call self%base%model%set_parameters(theta, status)
        ...
        call mlp_loss_value_gradient(self%base%model, self%base%validation_x, &
            self%base%validation_target, 0.0_dp, value, validation_gradient, &
            l2_gradient, status)
        if (status%code /= FORTNUM_OK) return
        do parameter_index = 1, MLP_ADAMW_FULL_HYPERPARAMETER_COUNT
        ...
        if (.not. ieee_is_finite(value) .or. any(.not. ieee_is_finite(gradient)) .or. &
            ...
            call status_set(status, FORTNUM_DOMAIN_ERROR, &
                "MLP full AdamW hypergradient: forward product is not finite")
            return
        end if
        call status_set(status, FORTNUM_OK, "")
    end subroutine adamw_full_forward
    ...
    logical function finite_adamw_full_parameters(parameters, learning_rate, l2, &
            weight_decay, beta1, beta2) result(valid)
        real(dp), intent(in) :: parameters(:)
        real(dp), intent(out) :: learning_rate, l2, weight_decay, beta1, beta2
        ...
        beta1 = 0.0_dp
        beta2 = 0.0_dp
        valid = size(parameters) == MLP_ADAMW_FULL_HYPERPARAMETER_COUNT .and. &
            all(ieee_is_finite(parameters))
        if (.not. valid) return
        learning_rate = exp(parameters(MLP_ADAMW_FULL_LOG_LEARNING_RATE))
        l2 = exp(parameters(MLP_ADAMW_FULL_LOG_L2))
        weight_decay = exp(parameters(MLP_ADAMW_FULL_LOG_WEIGHT_DECAY))
        beta1 = logit_to_probability(parameters(MLP_ADAMW_FULL_LOGIT_BETA1))
        beta2 = logit_to_probability(parameters(MLP_ADAMW_FULL_LOGIT_BETA2))
        valid = ieee_is_finite(learning_rate) .and. ieee_is_finite(l2) .and. &
            ieee_is_finite(weight_decay) .and. ieee_is_finite(beta1) .and. &
            ieee_is_finite(beta2) .and. learning_rate > 0.0_dp .and. l2 > 0.0_dp .and. &
            weight_decay > 0.0_dp .and. beta1 > 0.0_dp .and. beta1 < 1.0_dp .and. &
            beta2 > 0.0_dp .and. beta2 < 1.0_dp
    end function finite_adamw_full_parameters
    ...
    logical function valid_adamw_full_options(options) result(valid)
        type(mlp_adamw_full_hypergradient_options_t), intent(in) :: options
        ...
        valid = options%steps >= 1 .and. options%optimizer == MLP_OPTIMIZER_ADAMW .and. &
            ...
            ieee_is_finite(options%weight_decay) .and. options%learning_rate > 0.0_dp .and. &
            options%l2 > 0.0_dp .and. options%weight_decay > 0.0_dp .and. &
            ieee_is_finite(options%epsilon) .and. options%epsilon > 0.0_dp .and. &
            ieee_is_finite(options%lower_log_learning_rate) .and. &
            ieee_is_finite(options%upper_log_learning_rate) .and. &
            ieee_is_finite(options%lower_log_l2) .and. ieee_is_finite(options%upper_log_l2) .and. &
            ieee_is_finite(options%lower_log_weight_decay) .and. &
            ieee_is_finite(options%upper_log_weight_decay) .and. &
            ieee_is_finite(options%lower_logit_beta1) .and. &
            ...
            options%gradient_tolerance >= 0.0_dp .and. options%step_tolerance >= 0.0_dp .and. &
            options%objective_tolerance >= 0.0_dp
    end function valid_adamw_full_options
    ...
    pure real(dp) function logit_to_probability(logit) result(value)
        ...
        ! The accepted logit bounds keep this expression finite.  The
        ! direct logistic form is sufficient here and preserves the exact
        ! beta/logit derivative used by the hypergradient recurrence.
